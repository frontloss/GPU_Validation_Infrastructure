    ...
    internal class N10BitScanout : DeepColorBase
    {
        ...
        internal override Process Instance(IApplicationSettings argAppSettings, int argCurrMethodIdx)
        {
            ...
            if (null == playerProcess)
            {
                ...
                playerProcess.Start();
                if (base.DeepcolorParams.Bpc == 8)
                {
                    Log.Message("Enabling 8BPC from N10BitScanOut");
                    Log.Message("clicking f8");
        ...
        {
            string installerPath = string.Format("{0}\\{1}", base.AppSettings.DisplayToolsPath, DxSdk_Installer);
