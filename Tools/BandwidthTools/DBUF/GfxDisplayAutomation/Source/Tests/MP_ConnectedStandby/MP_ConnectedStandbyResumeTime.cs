namespace Intel.VPG.Display.Automation
{
    ...
    public class MP_ConnectedStandbyResumeTime : MP_ConnectedStandbyBase
    {
        ...
        public MP_ConnectedStandbyResumeTime()
        {
        ...
        public void StartStopProfiling()
        {
            ...
            for (int eachCycle = 1; eachCycle <= powerParam.Cycle; eachCycle++)
            {
                ...
                if (profileData.First().chronometerStatus)
                    Log.Success("Resumr time from CS as expected, Benchmark value {0}ms, Actual: {1}ms", data.benchMarkValue, Math.Round((data.actualValue) / 1000));
                else
                    Log.Fail("Resumr time from CS exceed the expectation, Benchmark Value: {0}ms, Actual: {1}ms", data.benchMarkValue, Math.Round((data.actualValue) / 1000));
                CaptureChronometerData();
                TimeTakenForEvent += Math.Round((profileData.First().actualValue) / 1000);
            }
            TimeTakenForEvent /= powerParam.Cycle;
            Log.Message("Average time taken for " + profileData.First().EventName + " for " + powerParam.Cycle + " cycle(s): " + Math.Round(TimeTakenForEvent, 2).ToString() + " ms");
        ...
        public void CaptureChronometerData()
        {
            ...
            LogData.Add(base.MachineInfo.Platform);
