...
{
    /*
     Check whether Hotplug unplug functionality is working correctly post S3
    */
    [Test(Type = TestType.HasPlugUnPlug)]
    class SB_DP_SST_HotPlug_Unplug_After_S3 : SB_DP_SST_Base
    {
        int delay = 15;
        PowerParams _powerParams = new PowerParams();

        //Put System into sleep state S3
        [Test(Type = TestType.Method, Order = 1)]
        public void putInS3()
        {
            //Delay can be modified as per requirement
            _powerParams.Delay = delay;
            //Put system into S3 state by invoking power event
            Log.Message("Putting System into S3");
            base.InvokePowerEvent(_powerParams, PowerStates.S3);
        }
