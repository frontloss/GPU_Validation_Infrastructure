...
{
    /*
    This testcase check whether Displays attached to system remain connected after S3
    */
   [Test(Type = TestType.HasPlugUnPlug)]
    class SB_DP_SST_S3 : SB_DP_SST_Base
    {
        int delay;
        ...
        public void verifyDPCD()
        {
            ...
            if (base.CurrentConfig.TertiaryDisplay != DisplayType.None)
            {
                ...
                {
                    Log.Alert("DP as Tertiary Display not connected post S3");
                }
            }
        }
        ...
        public void verifyInOSPage()
        {
            ...
            {
                Log.Message("All Displays are connected post S3");
            }
            else
            {
                Log.Fail("all displays before S3 are not same as all displays after S3");
            }
        }
     }
}
