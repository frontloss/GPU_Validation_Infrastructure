namespace Intel.VPG.Display.Automation
{
    ...
    public class MP_Chronometer_MonitorTurnOff : MP_ChronometerBase
    {
        ...
        public MP_Chronometer_MonitorTurnOff()
        {
            _logfilePath = "EVENT_MONITOR_TURN_OFF";
            cParam = new ChronometerParams();
            cParam.eventNameProfiling = EVENT_NAME_PROFILING.EVENT_MONITOR_TURN_OFF;
        ...
        public void StartStopProfiling()
        {
            ...
            try
            {
                for (int eachCycle = 1; eachCycle <= 1; eachCycle++)
                {
                    ...
                    Log.Message("Monitor Turn Off completed..");
                    ...
                    if (profileData.First().chronometerStatus)
                        Log.Success("Resumr time from Minitor Turn Off as expected, Benchmark value {0}ms, Actual: {1}ms", data.benchMarkValue, Math.Round((data.actualValue) / 1000));
                    else
                        Log.Alert("Resumr time from Minitor Turn Off exceed the expectation, Benchmark Value: {0}ms, Actual: {1}ms", data.benchMarkValue, Math.Round((data.actualValue) / 1000));
            ...
            TimeTakenForEvent = chronometerValues.First();
