namespace Intel.VPG.Display.Automation
{
    ...
    public class MP_Chronometer_S4_Enter: MP_ChronometerBase
    {
        ...
        public MP_Chronometer_S4_Enter()
        {
            cParam = new ChronometerParams();
            cParam.eventNameProfiling = EVENT_NAME_PROFILING.EVENT_ENTER_HIBERNATION;
            IsResumeTimeTest = true;
            _logfilePath = "EVENT_ENTER_HIBERNATION";
            ...
            try
            {
                for (int eachCycle = 1; eachCycle <= 6; eachCycle++)
                {
                    ...
                    if (profileData.First().chronometerStatus)
                        Log.Message("Resume time from CS as expected, Benchmark value {0}ms, Actual: {1}ms", data.benchMarkValue, Math.Round((data.actualValue) / 1000));
                    else
                        Log.Alert("Resume time from CS exceed the expectation, Benchmark Value: {0}ms, Actual: {1}ms", data.benchMarkValue, Math.Round((data.actualValue) / 1000));
