namespace Intel.VPG.Display.Automation
{
    ...
    class SB_DeepColor_Plug_Unplug_S4_SemiAutomated : SB_DeepColor_Base 
    {
        ...
        public void TestStep1()
        {
            ...
            if (_PluggedDisplay == null)
                Log.Abort("Please Connect Display Could not find New display connected");
        ...
        {
            if (!AccessInterface.SetFeature<bool, String>(Features.PromptMessage, Action.SetMethod, "Plug Non DeepColor Panel (Replace DP Panel) when system goes to S4"))
                Log.Abort("User rejected Semi Automated Request");

            Log.Message("Putting the system into {0} state & resume", PowerStates.S4);
            PowerParams powerParams = new PowerParams();
            powerParams.Delay = 15;
            base.InvokePowerEvent(powerParams, PowerStates.S4);
            Log.Success("Put the system into {0} state & resume", PowerStates.S4);
