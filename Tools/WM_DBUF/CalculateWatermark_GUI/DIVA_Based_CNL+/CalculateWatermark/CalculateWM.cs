...
namespace CalculateWatermark
{
    public partial class CalculateWM : Form
    {
        ...
        private void CalculateMemoryLatency()
        {
            ...
            WriteRegister((uint)0x138128, uMailbox_Data0);
            WriteRegister((uint)0x13812C, uMailbox_Data1);
            WriteRegister((uint)0x138124, uMailbox_Interface);
        ...
        private void CalculateArbitratedDisplayBandwidth()
        {
            ...
            {
                reg.ReadRegister((uint)0x147114, ref uMemoryFreqRegister);
                reg.ReadRegister((uint)0x147114, ref uMemoryChannel0);
                reg.ReadRegister((uint)0x147114, ref uMemoryChannel1);
            ...
            {
                reg.ReadRegister((uint)0x145E04, ref uMemoryFreqRegister);
                reg.ReadRegister((uint)0x14500C, ref uMemoryChannel0);
                reg.ReadRegister((uint)0x145010, ref uMemoryChannel1);
        ...
        internal bool WriteRegister(UInt32 dwOffset,UInt32 dwValue)
        {
            bool status = true;
            ...
            MmioAccessArgs.Offset = dwOffset;
            MmioAccessArgs.Value = dwValue;

            try
            {
                // Write MMIO
                DivaGfxAccessUtility.WriteMmio(MmioAccessArgs);
               // MessageBox.Show(string.Format("INFO: DIRECT-MMIO-Write @ Offset: 0x{0:X} = 0x{1:X}",
                //    MmioAccessArgs.Offset,
                //    MmioAccessArgs.Value));
            }
            catch (Exception Exp)
            {
               // MessageBox.Show(string.Format("ERROR: Exception in DIRECT-MMIO-WRITE: {0}",
               //     Exp.Message));
                status = false;
            }
            return status;
        ...
        internal bool ReadRegister(UInt32 dwOffset,ref UInt32 dwValue)
        {
            bool status = true;
