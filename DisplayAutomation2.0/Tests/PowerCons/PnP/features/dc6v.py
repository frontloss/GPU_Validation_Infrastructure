########################################################################################################################
# @file         dc6v.py
# @brief        Contains PnP tests for dc6v
# @details      PnP tests are covering Idle scenario by enabling and disabling feature
#               * this test compares C10 residency, hw power numbers with and without DC6v feature
#
...
##
# @brief        This is helper function to enable DC6V
...
##
# @brief        This is helper function to disable DC6V
# @param[in]    adapter Adapter object
# @return       True if the disabling is successful, False otherwise
def disable(adapter):
    status = dc_state.disable_dc6v(adapter)
    if status is False:
        return False
    if status is True:
        result, reboot_required = display_essential.restart_gfx_driver()
...
# @param[in]    bad_report
# @return       status
def analyze(tool, workload, good_report, bad_report):
    assert tool
    assert workload
    assert good_report
    assert bad_report
    ...
    good_dc_state_count = tool.get_metric(good_report, 'DC_STATE_COUNT')
    good_c0_score = tool.get_metric(good_report, 'PACKAGE_C0')
    good_c2_score = tool.get_metric(good_report, 'PACKAGE_C2')
    good_c8_score = tool.get_metric(good_report, 'PACKAGE_C8')
    good_c10_score = tool.get_metric(good_report, 'PACKAGE_C10')
    link_on = tool.get_metric(good_report, 'LINK_ON')
    link_off = tool.get_metric(good_report, 'LINK_OFF')
    good_io_requests = tool.get_metric(good_report, 'IO_READS') + tool.get_metric(good_report, 'IO_WRITES')

    logging.info("Bad Report: {0}".format(bad_report))
    bad_dc_state_count = tool.get_metric(bad_report, 'DC_STATE_COUNT')
    bad_c0_score = tool.get_metric(bad_report, 'PACKAGE_C0')
    bad_c2_score = tool.get_metric(bad_report, 'PACKAGE_C2')
    bad_c8_score = tool.get_metric(bad_report, 'PACKAGE_C8')
    bad_io_requests = tool.get_metric(bad_report, 'IO_READS') + tool.get_metric(bad_report, 'IO_WRITES')
    ...
    if good_c10_score > 0:
        logging.info("\tPASS: C10 is hitting when DC6v enabled - {0}%".format(good_c10_score))
    else:
        error_msg = "FAIL: C10 is not hitting when DC6v enabled".format(good_c10_score)
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: DC6v Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"DC6v Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            error_msg = f"Power regression in {key}: DC6v Enabled(W)- {round(good_pnp_data[key],4)}, " \
                        f"DC6v Disabled(W)- {round(bad_pnp_data[key],4)}"
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
