########################################################################################################################
# @file         hdr.py
# @brief        Contains PnP tests for hdr
# @details      PnP tests are covering hdr video playback scenario by enabling and disabling feature
#               * this test compares hw power numbers with and without HDR feature
#
...
##
# @brief        This is helper function to enable hdr
...
##
# @brief        This is helper function to disable hdr
# @param[in]    adapter Adapter object
# @return       True if the disabling is successful, False otherwise
def disable(adapter):
    status = blc.disable_hdr(adapter)
    if status is False:
        return False
    if status is True:
        result, reboot_required = display_essential.restart_gfx_driver()
...
##
# @brief        This is a helper function to analyse PnP with hdr
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: HDR Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"HDR Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            logging.error(f"Power regression in {key}: HDR Enabled(W)- {round(good_pnp_data[key],4)}, "
                          f"HDR Disabled(W)- {round(bad_pnp_data[key],4)}")
