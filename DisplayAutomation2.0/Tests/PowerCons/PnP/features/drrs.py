########################################################################################################################
# @file         drrs.py
# @brief        Contains PnP tests for drrs
# @details      PnP tests are covering Idle scenario by enabling and disabling feature
#               * this test compares C2 residency with and without DRRS feature
#
...
##
# @brief        This is helper function to disable DRRS
# @param[in]    adapter Adapter object
# @return       True if the disabling is successful, False otherwise
def disable(adapter):
    status = drrs.disable(adapter)
    if status is False:
        return False
    if status is True:
        result, reboot_required = display_essential.restart_gfx_driver()
...
##
# @brief        This is helper function to analyse C2 residency score in DRRS IDLE case
# @param[in]    workload string indicating the workload (IDLE_DESKTOP/SCREEN_UPDATE/...)
# @param[in]    good_report
# @param[in]    bad_report
# @return       status False if  C2 residency is high in DRRS IDLE case, True otherwise
def analyze(workload, good_report, bad_report):
    ...
    if workload == 'IDLE':
        if good_c2_score < bad_c2_score:
            logging.info("\tPASS: C2 residency is less in Idle case {0}%".format(good_c2_score))
        else:
            error_msg = "\tFAIL: C2 residency is high in DRRS Idle case {0}% DRRS disabled case {1}%".\
                format(good_c2_score, bad_c2_score)
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: DRRS Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"DRRS Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            error_msg = f"Power regression in {key}: DRRS Enabled(W)- {round(good_pnp_data[key],4)}, " \
                        f"DRRS Disabled(W)- {round(bad_pnp_data[key],4)}"
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
