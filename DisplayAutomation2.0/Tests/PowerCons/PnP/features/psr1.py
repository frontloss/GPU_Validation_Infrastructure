########################################################################################################################
# @file         psr1.py
# @brief        Contains PnP tests for psr1
# @details      PnP tests are covering Idle scenario by enabling and disabling feature
#               * this test compares c10, c8, residency with and without psr1 feature
#
...
GOLDEN_NUMBERS = {
    'ICLLP': {'C10': 0, 'Link_Off': 0, 'C8': 0},
    'TGL': {'C10': 0, 'Link_Off': 0, 'C8': 0},
    'ADLP': {'C10': 0, 'Link_Off': 0, 'C8': 0}
}


##
# @brief        This is helper function to enable psr
# @param[in]    adapter Adapter object
# @return       True if the enabling is successful, False otherwise
def enable(adapter):
    status = psr.enable(adapter.gfx_index, psr.UserRequestedFeature.PSR_1)
    if status is False:
        return False
    if status is True:
...
##
# @brief        This is helper function to disable psr
# @param[in]    adapter Adapter object
...
##
# @brief        This is a helper function to analyse PnP with psr1
...
def analyze(workload, good_report, bad_report):
    ...
    link_off = socwatch.get_metric(good_report, 'LINK_OFF')
    ...
    if workload == 'IDLE':
        if link_off > GOLDEN_NUMBERS[common.PLATFORM_NAME]['Link_Off']:
            logging.info("\tPASS: System is in PSR1 Link off state for {0}%".format(link_off))
            logging.info("\tPASS: System is in PSR1 Link on state for {0}%".format(link_on))
        else:
            error_msg = "FAIL: System is in PSR1 Link off state for {0}%".format(link_off)
            logging.info("\tSystem is in PSR1 Link on state for {0}%".format(link_on))
        ...
        if good_c10_score > GOLDEN_NUMBERS[common.PLATFORM_NAME]['C10']:
            logging.info("\tPASS: C10 hitting in PSR1 Idle case - {0}%".format(good_c10_score))
        else:
            error_msg = "FAIL: C10 not hitting in PSR1 Idle case - {0}%".format(good_c10_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]" + error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
        ...
        if bad_c8_score >= GOLDEN_NUMBERS[common.PLATFORM_NAME]['C8']:
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: PSR1 Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"PSR1 Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            error_msg = f"Power regression in {key}: PSR1 Enabled(W)- {round(good_pnp_data[key],4)}, " \
                        f"PSR1 Disabled(W)- {round(bad_pnp_data[key],4)}"
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
