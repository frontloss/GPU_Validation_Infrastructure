########################################################################################################################
# @file         dpst.py
# @brief        Contains PnP tests for dpst
# @details      PnP tests are covering Idle scenario by enabling and disabling feature
#               * this test compares C0 residency, hw power numbers with and without dpst feature
#
...
##
# @brief        This is helper function to enable dpst
...
##
# @brief        This is helper function to disable DPST
...
def analyze(workload, good_report, bad_report):
    ...
    if workload == 'SCREEN_UPDATE':
        if good_c0_score <= bad_c0_score:
            logging.info("\tPASS: C0 is less in DPST screen update case - {0}%".format(good_c0_score))
        else:
            error_msg = "FAIL: C0 is more in DPST screen update case {0}%".format(good_c0_score)
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: DPST Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"DPST Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            error_msg = f"Power regression in {key}: DPST Enabled(W)- {round(good_pnp_data[key],4)}, " \
                        f"DPST Disabled(W)- {round(bad_pnp_data[key],4)}"
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
