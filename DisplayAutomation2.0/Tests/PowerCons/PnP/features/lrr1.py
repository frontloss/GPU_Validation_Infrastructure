########################################################################################################################
# @file         lrr1.py
# @brief        Contains PnP tests for lrr1
# @details      PnP tests are covering Idle and 24fps video playback scenario by enabling and disabling feature
#               * this test compares PSR2 Deep sleep, Su, c10, c8, residency with and without lrr1 feature
#
...
##
# @brief        This is helper function to enable psr and lrr
...
##
# @brief        This is helper function to disable psr and lrr
# @param[in]    adapter Adapter object
...
##
# @brief        This is a helper function to analyse PnP with lrr1
...
def analyze(workload, good_report, bad_report):
    ...
    if workload == 'IDLE':
        ...
        if good_c10_score >= GOLDEN_NUMBERS[common.PLATFORM_NAME]['C10']:
            logging.info("\tPASS: C10 hitting in LRR1 Idle case - {0}%".format(good_c10_score))
        else:
            error_msg = "FAIL: C10 not hitting in LRR1 Idle case - {0}%".format(good_c10_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
        ...
        if bad_c8_score > 0:
            logging.info("\tPASS: C8 hitting in Non-LRR1 Idle case {0}%".format(bad_c8_score))
        else:
            error_msg = "FAIL: C8 is not hitting in Non-LRR1 Idle case {0}%".format(bad_c8_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
    ...
    if workload == 'SCREEN_UPDATE':
        ...
        if good_c10_score >= GOLDEN_NUMBERS[common.PLATFORM_NAME]['C10']:
            logging.info("\tPASS: C10 hitting in LRR1 screen update case - {0}%".format(good_c10_score))
        else:
            error_msg = "FAIL: C10 not hitting in LRR1 screen update case - {0}%".format(good_c10_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
        ...
        if bad_c8_score > 0:
            logging.info("\tPASS: C8 hitting in Non-LRR1 screen update case {0}%".format(bad_c8_score))
        else:
            error_msg = "FAIL: C8 is not hitting in Non-LRR1 screen update case {0}%".format(bad_c8_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
    ...
    if workload == 'VIDEO':
        ...
        if good_c8_score >= GOLDEN_NUMBERS[common.PLATFORM_NAME]['C8']:
            logging.info("\tPASS: C8 hitting in LRR1 video playback {0}%".format(good_c8_score))
        else:
            error_msg = "FAIL: C8 is not hitting in LRR1 video playback {0}%".format(good_c8_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: LRR1 Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"LRR1 Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            error_msg = f"Power regression in {key}: LRR1 Enabled(W)- {round(good_pnp_data[key],4)}, " \
                        f"LRR1 Disabled(W)- {round(bad_pnp_data[key],4)}"
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
