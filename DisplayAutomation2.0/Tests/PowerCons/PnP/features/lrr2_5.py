########################################################################################################################
# @file         lrr2_5.py
# @brief        Contains PnP tests for lrr2_5
# @details      PnP tests are covering Idle and 24fps video playback scenario by enabling and disabling feature
#               * this test compares PSR2 Deep sleep, Su, c10, c8, residency with and without lrr2_5 feature
#
...
##
# @brief        This is helper function to disable lrr 2.5
# @param[in]    adapter Adapter object
# @return       True if the disabling is successful, False otherwise
def disable(adapter):
    status = lrr.disable(adapter)
    if status is False:
        return False
    if status is True:
        result, reboot_required = display_essential.restart_gfx_driver()
...
##
# @brief        This is a helper function to analyse PnP with lrr 2.5
...
def analyze(workload, good_report, bad_report):
    ...
    if workload == 'IDLE':
        ...
        if good_c10_score >= GOLDEN_NUMBERS[common.PLATFORM_NAME]['C10']:
            logging.info("\tPASS: C10 hitting in LRR2.5 Idle case - {0}%".format(good_c10_score))
        else:
            error_msg = "FAIL: C10 not hitting in LRR2.5 Idle case - {0}%".format(good_c10_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
        ...
        if bad_c8_score > 0:
            logging.info("\tPASS: C8 hitting in Non-LRR2.5 Idle case {0}%".format(bad_c8_score))
        else:
            error_msg = "FAIL: C8 is not hitting in Non-LRR2.5 Idle case {0}%".format(bad_c8_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
    ...
    if workload == 'SCREEN_UPDATE':
        ...
        if good_c10_score >= GOLDEN_NUMBERS[common.PLATFORM_NAME]['C10']:
            logging.info("\tPASS: C10 hitting in LRR2.5 screen update case - {0}%".format(good_c10_score))
        else:
            error_msg = "FAIL: C10 not hitting in LRR2.5 screen update case - {0}%".format(good_c10_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
        ...
        if bad_c8_score > 0:
            logging.info("\tPASS: C8 hitting in Non-LRR2.5 screen update case {0}%".format(bad_c8_score))
        else:
            error_msg = "FAIL: C8 is not hitting in Non-LRR2.5 screen update case {0}%".format(bad_c8_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
    ...
    if workload == 'VIDEO':
        ...
        if good_c8_score >= GOLDEN_NUMBERS[common.PLATFORM_NAME]['C8']:
            logging.info("\tPASS: C8 hitting in LRR2.5 video playback {0}%".format(good_c8_score))
        else:
            error_msg = "FAIL: C8 is not hitting in LRR2.5 video playback {0}%".format(good_c8_score)
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
                problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: LRR2.5 Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"LRR2.5 Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            error_msg = f"Power regression in {key}: LRR2.5 Enabled(W)- {round(good_pnp_data[key],4)}, " \
                        f"LRR2.5 Disabled(W)- {round(bad_pnp_data[key],4)}"
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
