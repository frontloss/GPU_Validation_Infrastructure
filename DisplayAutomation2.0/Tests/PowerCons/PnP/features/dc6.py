########################################################################################################################
# @file         dc6.py
# @brief        Contains PnP tests for dc6
# @details      PnP tests are covering Idle scenario by enabling and disabling feature
#               * this test compares C10 residency, hw power numbers with and without DC6 feature
#
...
##
# @brief        This is helper function to disable DC6
# @param[in]    adapter Adapter object
# @return       True if the disabling is successful, False otherwise
def disable(adapter):
    status = dc_state.disable_dc6(adapter)
    if status is False:
        return False
    if status is True:
        result, reboot_required = display_essential.restart_gfx_driver()
...
##
# @brief        This is a helper function to analyse PnP with DC6
...
def analyze(workload, good_report, bad_report):
    ...
    if good_c10_score > 0:
        logging.info("\tPASS: C10 is hitting when DC6 enabled - {0}%".format(good_c10_score))
    else:
        error_msg = "FAIL: C10 is not hitting when DC6 enabled".format(good_c10_score)
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: DC6 Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f"DC6 Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            error_msg = f"Power regression in {key}: DC6 Enabled(W)- {round(good_pnp_data[key],4)}, " \
                        f"DC6 Disabled(W)- {round(bad_pnp_data[key],4)}"
            logging.error(error_msg)
            gdhm.report_bug(
                title="[PnP]"+error_msg,
