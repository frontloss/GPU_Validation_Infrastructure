########################################################################################################################
# @file         lace.py
# @brief        Contains PnP tests for lace
...
##
# @brief        This is helper function to enable lace
# @param[in]    adapter Adapter object
# @return       True if the enabling is successful, False otherwise
def enable(adapter):
    for adapter in dut.adapters.values():
        for panel in adapter.panels.values():
            pc_external.enable_disable_lace(adapter, panel, True)


##
# @brief        This is helper function to disable lace
# @param[in]    adapter Adapter object
# @return       True if the disabling is successful, False otherwise
def disable(adapter):
    for adapter in dut.adapters.values():
        for panel in adapter.panels.values():
            pc_external.enable_disable_lace(adapter, panel, False)


##
# @brief        This is a helper function to analyse PnP with lace
...
def analyze_hw(good_report, bad_report):
    ...
    for key in good_pnp_data:
        logging.info(f"{key}: LACE Enabled(W)- {round(good_pnp_data[key],4)}, "
                     f":LACE Disabled(W)- {round(bad_pnp_data[key],4)}")
        if good_pnp_data[key] > bad_pnp_data[key]:
            logging.error(f"Power regression in {key}: LACE Enabled(W)- {round(good_pnp_data[key],4)}, "
                          f"LACE Disabled(W)- {round(bad_pnp_data[key],4)}")
