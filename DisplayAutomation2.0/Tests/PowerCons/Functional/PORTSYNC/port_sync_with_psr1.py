#######################################################################################################################
# @file         port_sync_with_psr1.py
# @addtogroup   Powercons
# @section      PORTSYNC
# @brief        Test for port sync in symmetric Dual eDP with PSR1
#
...
##
# @brief        This class contains basic port sync tests with PSR1
class PortSyncWithPsr1(PortSyncBase):
    ############################
    # Test Function
    ############################
    ##
    # @brief        this function verifies port sync with PSR1
    # @return       None
    def t_10_test_port_sync_psr1(self):
        self.verify_basic()
    ...
    def verify_basic(self):
        offsets = psr.get_polling_offsets(psr.UserRequestedFeature.PSR_1)
                ...
                if len(self.lfp_panels) == 2:
                    ...
                    for panel in self.lfp_panels:
                        if panel.psr_caps.is_psr_supported:
                            if psr.verify(adapter, panel, psr.UserRequestedFeature.PSR_1 , etl_file, polling_data, 'APP',
                                          False) is False:
                                logging.error("\tFAIL: PSR1 verification failed on {0}".format(panel))
                                self.fail("\tPSR verification failed failed with port sync")
                        else:
                            self.fail("\tNon Psr1 panel connected(Planning Issue)")
            else:
...
if __name__ == '__main__':
    ...
    test_result = runner.run(common.get_test_suite(PortSyncWithPsr1))
