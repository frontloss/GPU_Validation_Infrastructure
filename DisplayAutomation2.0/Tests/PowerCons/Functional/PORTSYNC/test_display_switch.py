...
##
# @brief        This class contains basic port sync tests in display switch scenario
class TestDisplaySwitch(PortSyncBase):
    ##
    # @brief        this function verifies port sync in display switch scenario
    # @return       None
    def t_10_display_switch(self):
        ...
        for adapter in dut.adapters.values():
            ...
            # Display configs ex:- SD EDP A, SD EDP B, DDC, DDE
            if len(self.ext_panels) == 0 and len(self.lfp_panels) == 2:
                config_list = [(enum.SINGLE, [self.lfp_panels[0].port], [self.lfp_panels[0]]),
                               (enum.EXTENDED, [self.lfp_panels[0].port, self.lfp_panels[1].port],
                                [self.lfp_panels[0], self.lfp_panels[1]]),
                               (enum.SINGLE, [self.lfp_panels[1].port], [self.lfp_panels[1]]),
                               (enum.EXTENDED, [self.lfp_panels[1].port, self.lfp_panels[0].port],
                                [self.lfp_panels[1], self.lfp_panels[0]]),
                               (enum.SINGLE, [self.lfp_panels[0].port], [self.lfp_panels[0]]),
            ...
            for config in config_list:
                if self.display_config_.set_display_configuration_ex(config[0], config[1]) is False:
                    self.fail("Applying Display config {0} {1} Failed".
                              format(DisplayConfigTopology(config[0]).name, config[1]))
