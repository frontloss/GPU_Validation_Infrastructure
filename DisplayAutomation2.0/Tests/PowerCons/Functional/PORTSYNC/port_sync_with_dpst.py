#######################################################################################################################
# @file         port_sync_with_dpst.py
# @addtogroup   Powercons
# @section      PORTSYNC
# @brief        Test for port sync in symmetric Dual eDP with DPST,
#
...
##
# @brief        Contains basic port sync DPST tests
class TestPortSyncDPST(PortSyncBase):
    ############################
    # Test Function
    ############################

    ##
    # @brief        this function verifies port sync test with DPST concurrency
    # @return       None
    # @cond
    @common.configure_test(repeat=True)
    # @endcond
    def t_10_test_port_sync_dpst(self):
                ...
                if len(self.lfp_panels) == 2:
                    ...
                    for panel in self.lfp_panels:
                        if dpst.verify(adapter, panel, etl_file) is False:
                            self.fail("DPST verification failed on {} with CMTG mode")
                    logging.info("\tPort sync functional verification successful with DPST")
            else:
...
if __name__ == '__main__':
    ...
    test_result = runner.run(common.get_test_suite(TestPortSyncDPST))
