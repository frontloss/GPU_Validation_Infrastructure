#######################################################################################################################
# @file         override_bpc_encoding_plug_unplug.py
# @addtogroup   Test_Color
# @section      override_Bpc_encoding
# @remarks      @ref override_bpc_encoding_plug_unplug.py \n
#               The test script applies the modeset as 19*10 or 4K or 4k HDR invokes the driver escape
...
#               to persist after performing hotplug and unplug event
...
class OverrideBpcEncodingPlugUnplug(ColorCommonBase):
            ...
            if self.enumerated_displays.ConnectedDisplays[display_index].IsActive:
                ...
                for index in range(len(combo_bpc_encoding)):
                    ...
                    if status:
                        ...
                        if color_common_utility.hotunplug_plug_display_etl_trace(self.cmd_line_param, cmd_parser,self.connected_list) is False:
                            self.fail("Failed to perform hotplug-unplug event")
                        logging.info("Pass: Hotplug-unplug event was success")
                        time.sleep(5)

                        if verify_bpc_pixel_encoding_register_programming(pipe, self.platform, avi_encoding_mode, bpc, encoding):
                            logging.info("Pass: Register verification success after performing hotplug-unplug event for current BPC and Encoding")
                        else:
                            self.fail("Fail: Register verification failed after performing hotplug-unplug event for current BPC and Encoding")
                    else:
