#######################################################################################################################
# @file         override_bpc_encoding_powerevents.py
# @addtogroup   Test_Color
# @section      override_Bpc_encoding
# @remarks      @ref override_bpc_encoding_powerevents.py \n
...
# #               to persist after performing after power state S3.
...
class OverrideBpcEncodingPowerEvents(ColorCommonBase):
            ...
            if self.enumerated_displays.ConnectedDisplays[display_index].IsActive:
                ...
                for index in range(len(combo_bpc_encoding)):
                    ...
                    if status:
                        ...
                        if color_common_utility.invoke_power_states(display_power.PowerEvent.S3) is False:
                            self.fail("Failed to invoke power event")
                        else:
                            logging.info("Pass: Power event S3 completed")

                        time.sleep(5)

                        if verify_bpc_pixel_encoding_register_programming(pipe, self.platform, avi_encoding_mode, bpc, encoding):
                            logging.info("Pass: Register verification success after performing power event for current BPC and Encoding")
                        else:
                            self.fail("Fail: Register verification failed after performing power event for current BPC and Encoding")
                    else:
