#######################################################################################################################
# @file           os_hdr_plug_unplug.py
# @addtogroup     Test_Color
# @section        os_hdr_plug_unplug
# @remarks        @ref os_hdr_plug_unplug.py \n
#                 Test for verifying the persistence of HDR with Hotplug and Unplug event
#                 Basic register verification (HDR Mode in Pipe_Misc register) will be performed.
#                 To-Do : Add ETL Parsing for Metadata
# Command Line :  python os_hdr_plug_unplug.py -edp_a SINK_EDP050 -hdmi_b SamsungJS9500_HDR.bin -config EXTENDED
#                 python os_hdr_plug_unplug.py -edp_a SINK_EDP050 -dp_d  Benq_SW320.bin  DP_HDR_DPCD.txt  -CONFIG
#                 EXTENDED
# @author         Vimalesh D
#######################################################################################################################
import time
from Libs.Core.test_env.test_environment import TestEnvironment
from Tests.Color.HDR.OSHDR.os_hdr_base import *
from Tests.Color.color_common_base import *
from Tests.Color import color_common_utility
from Tests.Color.HDR.OSHDR import os_hdr_verification


class OSHDRHotPlugUnplug(OSHDRBase):

    connected_disp_index = 0
    connected_disp_target_id = 0
    os_hdr_verify = os_hdr_verification.OSHDRVerification()

    def runTest(self):

        event_name = "Hotplug_Unplug_event"
        ##
        # Check Power Mode  for DC or AC to check the HDR Option temporarily disabled
        result, status = color_common_utility.check_and_apply_power_mode()
        if status:
            logging.info(result)
        else:
            self.fail(result)

        ##
        # Enable OS_HDR on all active displays
        self.enumerated_displays = self.config.get_enumerated_display_info()

        super().toggle_and_verify_hdr(toggle="ENABLE")

        time.sleep(5)

        ##
        # Apply 4K Modeset with refresh rate 60 hz.But based on supported modes it will apply,So ignoring this.
        super().apply_native_mode()

        ##
        # Hotunplug and plug activity
        if color_common_utility.start_etl_capture("Hot_Unplug_and_plug") is False:
            self.fail("GfxTrace failed to start")

        if color_common_utility.hotunplug_plug_display_etl_trace(self.cmd_line_param, cmd_parser,
                                                                 self.connected_list) is False:
            color_common_utility.stop_etl_capture("Hot_Unplug_and_plug")
            self.fail("Failed to hotunplug and  plug display event")
        else:
            logging.info("Finished hotunplug and plug display event")

        color_common_utility.stop_etl_capture("Hot_Unplug_and_plug")
        ##
        # Verify HDR persistence after HotUnplug and plug display
        enumerated_displays = self.config.get_enumerated_display_info()
        if self.os_hdr_verify.verify_hdr_persistence(event_name, self.enumerated_displays, self.platform, self.connected_list) is False:
            self.fail("Failed to verify HDR persistence after unplug and plug display call")

        ##
        # Disable HDR
        self.enumerated_displays = self.config.get_enumerated_display_info()
        super().toggle_and_verify_hdr(toggle="DISABLE")


if __name__ == '__main__':
    TestEnvironment.initialize()
    outcome = unittest.main(exit=False, argv=[sys.argv[0]])
    TestEnvironment.cleanup(outcome.result)
