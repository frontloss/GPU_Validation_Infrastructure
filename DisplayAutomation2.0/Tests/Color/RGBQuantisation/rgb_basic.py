...
# CommandLine:  python rgb_basic.py  -HDMI_B HDMI_DELL.EDID -CONFIG SINGLE -LOGLEVEL DEBUG
# @author       Vimalesh D
#######################################################################################################################
import time
...
class RGBBasic(ColorCommonBase):
    ...
    def runTest(self):
        ...
        for display_index in range(self.enumerated_displays.Count):
            ...
            if self.enumerated_displays.ConnectedDisplays[display_index].IsActive:
                ...
                pipe = color_common_utility.get_current_pipe(str(CONNECTOR_PORT_TYPE(self.enumerated_displays.ConnectedDisplays[display_index].ConnectorNPortType)))
                ...
                for resolution in range(0, len(self.res_conv_type_dict)):
                    ...
                    if self.modeset_status is not None:
                        ...
                        for set_range in range(0, len(set_quant_list)):
                            ...
                            output, status = color_common_utility.get_set_avi_info(self.enumerated_displays,display_index,self.avi_info, set_quant_list[set_range])
                            if status:
                                logging.info(output)
                            else:
                                self.fail("Failed to set  quantisation range")
