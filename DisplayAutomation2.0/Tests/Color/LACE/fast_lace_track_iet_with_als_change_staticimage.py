#######################################################################################################################
# @file         fast_lace_track_iet_with_als_change_staticimage.py
# @addtogroup   Test_Color
# @section      fast_lace_track_iet_with_als_change_staticimage.py
# @remarks      @ref fast_lace_track_iet_with_als_change_staticimage.py \n
#               The test script invokes Driver PCEscape call to enable LACE using Lux .
#               For different static images the IET computed is verified to have changed for each image with als change
# @author       Soorya R
#######################################################################################################################
import win32api
...
class LACETrackIETWithALSChangeStaticImage(LACEBase):

    def runTest(self):
        IMAGE_FILE = "driveway.bmp"
        LUX_VALUES = [551, 4000, 18000, 600]
        iet_data_list = []
        ...
        ##
        # Verify lace programming for enable
        ...
        for index in range(0, 4):
            ...
            if driver_escape.als_aggressiveness_level_override(display_and_adapter_info=self.lfp_target_ids[0],
                                                               lux=LUX_VALUES[index],
                                                               lux_operation=True):
                logging.info(" Als Lux override set successfully - Lux : %d" % LUX_VALUES[index])
            time.sleep(5)
        ...
        ##
        # Verify lace programming for disable
        for index in range(len(self.lfp_target_ids)):
