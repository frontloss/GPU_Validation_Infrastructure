########################################################################################################################
# @file         igcl_set_ftr_loop.py
# @brief        Test script which helps to set different blocks in different Color Modes in a loop
        ...
        for index in range(0, 30):
            for gfx_index, adapter in self.context_args.adapters.items():
                for port, panel in adapter.panels.items():
                    status, igcl_get_caps = igcl_color_cc_block.fetch_igcl_color_ftrs_caps_and_verify(gfx_index,
                                                                                                      adapter.platform,
                    ...
                    if color_igcl_escapes.set_igcl_color_feature(igcl_set_args, igcl_get_caps, panel.target_id,
                        ...
                        logging.error("FAIL: IGCL Escape call for SET is unsuccessful")
                        self.fail()
                    else:
                        logging.info("PASS: IGCL Escape call for SET is successful")
