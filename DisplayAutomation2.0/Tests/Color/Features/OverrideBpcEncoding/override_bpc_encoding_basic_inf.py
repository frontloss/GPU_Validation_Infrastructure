#################################################################################################
# @file         override_bpc_encoding_basic_inf.py
...
##
# @brief - To perform basic verification using OEM RegKey
            ...
            for port, panel in adapter.panels.items():
                if panel.is_active and panel.is_lfp:
                    ...
                    if verify(adapter.gfx_index, adapter.platform, panel.pipe, plane_id,
                        ...
                        logging.info("Successfully verified the override bpc {0} and encoding {1} using OEM RegKey".format(bpc, encoding))
...
if __name__ == '__main__':
    ...
    logging.info("Test purpose: Configure 10 BPC and encoding for the LFP panel using OEM RegKey")
