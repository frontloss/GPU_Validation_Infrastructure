...
# \section ycbcr_ddc_overlay_yuv_rgb
# \remarks
# \ref ycbcr_ddc_overlay_yuv_rgb.py \n
# This script performs color functionality such as enable/disable YCbCr on the supported panel
# when the overlay application is playing in Clone configuration.
# It also checks whether the output colorspace is YUV when YCbCr is enabled and
# RGB when YCbCr is disabled
#
# CommandLine: python ycbcr_ddc_overlay_yuv_rgb.py -hdmi_b HDMI_Dell_U2709_YCBCR.EDID -edp_a 
#
...
class YCbCrDDCOverlayYUVRGB(ColorBase):

    def verify_registers(self, output_colorspace):
        #
        # Get the output colorspace
        logging.info(self.getStepInfo() + "Verifying output color space for display %s" % self.display)
        colorspace_status = get_pipe_output_colorspace(self.display, 'PIPE_MISC', output_colorspace)
        if colorspace_status != output_colorspace:
    ...
    def runTest(self):
        ...
        if ycbcr_supported:
            ...
            if self.config.set_display_configuration_ex(topology, self.connected_list):
                ...
                if colorspace_status != "RGB":
                    self.fail("Pipe output color space is not RGB: %s" % colorspace_status)

                ##
                # Enables YCbCr
                logging.info(self.getStepInfo() + "Enabling YCbCr on %s panel" % self.display)
                ...
                self.fail("Failed to connect display configuration: %s %s" % (DisplayConfigTopology(topology).name,
                                                                              self.connected_list))
        else:
...
if __name__ == '__main__':
    ...
    logging.info(
        "Test purpose: Enables YCbCr on supported panels with overlay application running. Checks for YUV output "
        "color space when YCbCr is enabled in clone configuration")
