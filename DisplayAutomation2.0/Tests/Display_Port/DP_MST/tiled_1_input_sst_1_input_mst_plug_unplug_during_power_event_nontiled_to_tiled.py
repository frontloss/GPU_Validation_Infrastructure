################################################################################################################
# @file          tiled_1_input_sst_1_input_mst_plug_unplug_during_power_event_nontiled_to_tiled.py
...
##
# @brief        This class contains runTest() method that verifies switch of SST panel and MST tiled panel to SST  tiled
#               panel during power event
class DPMSTTiledOneInputSSTOneInputMSTHotplugUnplugPowerEventsNonTiledtoTiled(DisplayPortMSTBase):

    ##
    # @brief        This method executes the actual test steps. It plugs the DP SST Panel and DP MST tiled panel
    #               mentioned as 1st and 2nd plug_topolgy argument in commandline respectively, applies max mode and
    #               performs tiled display verification.Then it unplugs MST tiled display topology from slave port and
    #               to same port it plugs the SST tiled display mentioned as 3rd plug_topology argument in commandline.
    #               Then invokes supported power event(s3/cs). After resuming from power event it applies max mode and
    ...
    def runTest(self):
        ...
        self.set_low_power_state(1, port_type2, enum.UnPlugOldPlugNew, True, topology_type3)

        # Send the SST XML Data to Gfx Simulation Driver
        self.parse_send_topology(port_type2, topology_type3, xml_file3, True)
