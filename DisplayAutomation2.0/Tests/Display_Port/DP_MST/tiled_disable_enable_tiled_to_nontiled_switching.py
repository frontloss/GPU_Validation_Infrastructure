################################################################################################################
# @file          tiled_disable_enable_tiled_to_nontiled_switching.py
# @brief         Verify able to unplug SST tiled panel and able to plug back MST panel at the same port.
...
##
# @brief        This class contains runTest() method that verifies switching from sst tiled topology to mst tiled at
#               same port during driver disable/enable
class DPMSTTiledDisableEnableTiledtoNonTiledSwitiching(DisplayPortMSTBase):

    ##
    # @brief        This method executes the actual test steps.It plugs 1st SST tiled panel to 1st port, and 2nd SST
    #               tiled panel to 2nd port.It applies max mode and performs tiled display verification and then unplugs
    #               sst tiled topology from 2nd port.Driver is restarted and then 3rd topology(MST Tiled) from the
    #               command line is plugged to 1st port.Max mode is applied and tiled display verification is performed
    # @return       None
    def runTest(self):
        ...
        # Unplug Slave Port Type
        self.set_hpd(port_type2, False, True)

        # Set SD config and set non-tiled max(4K) mode
        self.set_config_apply_max_mode()
