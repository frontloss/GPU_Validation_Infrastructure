########################################################################################################################
# @file         flipq_multi_frame.py
# @brief        Test submits queued flips with different PTS values which are very close by and verify for flip execution.
#               * Create planes with different plane parameters.
#               * Submit All param flip.
#               * Submit 8 address only flips in which 3 flips are executed within a frame.
...
##
# @brief    Contains function to check for the hardware support for the plane parameters and flip the content in multiframe
class FlipQMultiFrame(flipq_base.FlipQBase):
    ...
    def flipq_perform_flip(self, pplanes):
        ...
        if (checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS):
            ...
            ##
            # Target flip time list with nearby frames
            kernel32.QueryPerformanceCounter(ctypes.byref(flip_time))
            tft = flip_time.value + 100000000
            delay_list = [0, 2, 3, 4, 100, 200, 300, 400, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000]
    ...
    # @return           void
    def tearDown(self):
        super(FlipQMultiFrame, self).tearDown()
        mpo = flip.MPO()
        for i in range(0, 2):
...
if __name__ == '__main__':
    ...
    logging.info("Test Purpose: Submit queued flips with different PTS values which are very close by "
                 "and verify for flip execution.")
