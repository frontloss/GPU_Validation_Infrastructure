########################################################################################################################
# @file         flipq_full_queue.py
# @brief        Test queues the flips with later time stamps until the queue is full and then submit another queued flip
#               once the first flip in the queue is executed and verify the flip execution time.
#               * Create planes with different plane parameters.
#               * Submit All param flip.
#               * Submit 8 address only flips.
#               * Submit another address only flip once the first flip in the queue is executed.
...
##
# @brief    Contains function to check for the hardware support for the plane parameters and flip the content in queue
class FlipQFullQueue(flipq_base.FlipQBase):
    ...
    def flipq_perform_flip(self, pplanes):
        ...
        if (checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS):
            ...
            delay = 1000000
            for index in range(0, planes_helper.SIMPLE_FLIPQ_DEPTH + 1):
            ...
            for index in range(0, planes_helper.SIMPLE_FLIPQ_DEPTH):
                ...
                if pplanes.ulTargetFlipTime not in target_flip_time_list:
                    target_flip_time_list.append(pplanes.ulTargetFlipTime)
            ...
            ##
            # Wait till the execution of first queue element is completed
            while True:
                kernel32.QueryPerformanceCounter(ctypes.byref(flip_time))
                if flip_time.value >= target_flip_time_list[0]:
                    break

            ##
            # Wait for first flip to get executed
            time.sleep(planes_helper.SCANLINE_MODE_WAIT)

            ##
            # Queue another element after first flip is executed
            resource_in_use = (1, 0)[resource_in_use]
            for plane_index in range(0, pplanes.uiPlaneCount):
                pplanes.stPlaneInfo[plane_index].iResourceInUse = resource_in_use
            kernel32.QueryPerformanceCounter(ctypes.byref(flip_time))
            delay = flip_time.value + delay_list[-1]
            pplanes.ulTargetFlipTime = delay
            ssa_result = self.mpo.flipq_set_source_address_mpo3(pplanes)
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                logging.info("Successfully queued the flips")
                for display_index in range(0, self.no_of_displays):
            ...
            for display in self.connected_list:
                if planes_helper.verify_flipq_mmio(etl_file, display, 0, planes_helper.SIMPLE_FLIPQ_DEPTH + 1):
    ...
    # @return           void
    def tearDown(self):
        super(FlipQFullQueue, self).tearDown()
        mpo = flip.MPO()
        for i in range(0, 2):
...
if __name__ == '__main__':
    ...
    logging.info("Test Purpose: Queue the flips with later time stamps until the queue is full and "
                 "then submit another queued flip once the first flip in the queue is executed.")
