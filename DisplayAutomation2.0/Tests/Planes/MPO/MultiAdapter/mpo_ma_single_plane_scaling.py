########################################################################################################################
# @file         mpo_ma_single_plane_scaling.py
# @brief        Basic test to flip single plane of specified formats on each display across multiple adapters and perform
...
##
# @brief    Contains function to flip single plane of specified formats on each display and perform plane scaling
class MPOMASinglePlaneScaling(MPOMABase):
    ...
    def runTest(self):
        ...
        for key, value in self.display_details.items():
            ...
            for source_id in range(0, len(value)):
                plane1 = flip.PLANE_INFO(source_id, 0, 1, self.pixel_format[index], self.tile_format[index], rect, rect,
            ...
            for ddimension in self.dst_list:
                ...
                for key, value in self.display_details.items():
                    ...
                    for index in range(0, pplanes[adapter_index].uiPlaneCount):
                        ...
                        pplanes[adapter_index].stMPODstRect = dst_dimension
...
if __name__ == '__main__':
    ...
    logging.info("Test Purpose: To flip single plane of specified formats on each display across multiple adapters"
