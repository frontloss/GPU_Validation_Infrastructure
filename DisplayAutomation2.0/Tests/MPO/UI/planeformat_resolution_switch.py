########################################################################################################################
# @file         planeformat_resolution_switch.py
# @brief        Basic test to verify plane format getting enabled with no corruption on video app during mode change
#               across all the displays connected.
...
##
# @brief    Contains function to check plane format getting enabled with no corruption on video app during mode change
class PlaneFormatResolutionSwitch(mpo_ui_base.MPOUIBase):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                if not self.mpo_helper.verify_planes(self.connected_list[display_index], 'PLANE_CTL_1', plane1_pixelformat):
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed during media playback application",
