########################################################################################################################
# @file         mpo_3d_app_display_switch_tiled_sst.py
# @brief        Basic test to verify register programming and also check for underrun while running 3D app and display
#               switch scenario on Tiled Display and non tiled display.
...
#               * Run 3D application.
#               * Apply different display configurations.
...
class MPO3DAppDisplaySwitchTiledSST(mpo_ui_tiled_sst_base.MPOUITiledSSTBase):
    display_list = []
    config_list = []
                    ...
                    for index in range(0, len(self.config_list[each_config][1])):
                        ...
                        if tile_info.TiledStatus is True and index == 0:
                            ...
                            if not self.mpo_helper.verify_planes_sst(pixel_format, 'PLANE_CTL_1'):
                                gdhm.report_bug(
                                    title="[MPO]Plane verification failed while running 3D app in fullscreen "
                                          "on Tiled display",
                                ...
                                self.fail("Plane verification failed while running 3D app in fullscreen "
                                          "on Tiled display")
                            else:
                                logging.info("Plane verification passed while running 3D app in fullscreen "
                                             "on Tiled display")
                            ...
                            if not self.mpo_helper.verify_planes(display, 'PLANE_CTL_1', pixel_format):
                                gdhm.report_bug(
                                    title="[MPO]Plane verification failed during while running 3D app in fullscreen "
                                          "on non Tiled display",
                                    problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                                    component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                                    priority=gdhm.Priority.P2,
                                    exposure=gdhm.Exposure.E2
                                )
                                self.fail("Plane verification failed during while running 3D app in fullscreen "
                                          "on non Tiled display {}" .format(display))
                            else:
                                logging.info("Plane verification passed while running 3D app in fullscreen "
                                             "on non Tiled display {}" .format(display))
                            logging.info("*************************Plane verification ended*************************")
...
if __name__ == '__main__':
    ...
    logging.info("Test purpose: To verify for register programming and also check for underrun while running"
                 "3D app and display switch scenario on Tiled Display and non tiled display")
