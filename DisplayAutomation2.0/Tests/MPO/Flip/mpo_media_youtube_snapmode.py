...
# @brief        Basic test to verify underruns during resize of 2 YUV planes in snapmode.
#               * Apply Single display configuration across all the displays.
#               * Submit flips with 3 planes where 2 planes are of YUV_422 format and 1 plane is of RGB format in snapmode.
#                 RGB format represents the desktop and 3D, YUV422 format represents media content and youtube playback.
...
##
# @brief    Contains function to check for underruns during resize of 2 YUV planes in snapmode
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                logging.info(
                    self.mpo_helper_dft.getStepInfo() + 'Performed flip of 3 planes: YUV_422_PACKED_8_BPC, YUV_422_PACKED_8_BPC, RGB_8888')
                ##
                ...
                if not plane1_result or not plane2_result or not plane3_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: YUV_422_PACKED_8_BPC, YUV_422_PACKED_8_BPC, RGB_8888",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: YUV_422_PACKED_8_BPC, YUV_422_PACKED_8_BPC, RGB_8888')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Right", -3)):
