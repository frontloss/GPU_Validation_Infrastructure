########################################################################################################################
# @file         mpo_media_youtube_snapmode.py
# @brief        Basic test to verify underruns during resize of 2 NV12 planes in snapmode.
#               * Apply Single display configuration across all the displays.
#               * Submit flips with 3 planes where 1 plane is of RGB format and 2 planes are of NV12 format in snapmode.
#                 RGB format represents the desktop, NV12 format represents media content and youtube playback.
...
##
# @brief    Contains function to check underruns during resize of 2 NV12 planes in snapmode
class MPOMediaYoutubeSnapMode(mpo_base.MPOBase):
    display = None
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                logging.info(self.mpo_helper_dft.getStepInfo() + 'Performed flip of 3 planes: NV12_YUV_420, NV12_YUV_420, RGB_8888')
                ##
                ...
                if not plane1_result or not plane2_result or not plane3_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: NV12_YUV_420, NV12_YUV_420, RGB_8888",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: NV12_YUV_420, NV12_YUV_420, RGB_8888')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                rect3 = flip.MPO_RECT(0, 0, 1920, resolution.VtRes)
...
if __name__ == '__main__':
    ...
    logging.info('Test purpose: Flips NV12_YUV_420 and RGB planes in snap mode checks for flickr/corruption')
