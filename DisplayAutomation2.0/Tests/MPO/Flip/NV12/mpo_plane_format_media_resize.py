    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Bottom", 4)):
                    self.perform_flip(pplanes)

                    self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Right", 4)
                    self.perform_flip(pplanes)
...
if __name__ == '__main__':
    ...
    logging.info(
        'Test purpose: Flips NV12_YUV_420 and RGB planes checks for watermark & underrun errors  during resize')
