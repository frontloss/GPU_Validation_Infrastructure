...
#               * Submit flips with 3 planes where 2 planes are of RGB format and 1 plane is of NV12 format.
#                 RGB format represents the desktop and 3D application, NV12 format represents media content.
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                logging.info(self.mpo_helper_dft.getStepInfo() + 'Performed flip of 3 planes: NV12_YUV_420, RGB_8888, RGB_8888')
                ##
                ...
                if not plane1_result or not plane2_result or not plane3_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: NV12_YUV_420, RGB_8888, RGB_8888",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: NV12_YUV_420, RGB_8888, RGB_8888')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                plane3 = flip.PLANE_INFO(0, 2, 1, flip.PIXEL_FORMAT.PIXEL_FORMAT_NV12YUV420,
                ...
                ##
                # Assigning different plane co-ordinates for portrait and landscape panels
                if resolution.HzRes < resolution.VtRes:
                    pplanes.stPlaneInfo[2].stMPODstRect.lLeft = 400
                    pplanes.stPlaneInfo[2].stMPOClipRect.lLeft = 400
                    pplanes.stPlaneInfo[2].stMPODstRect.lTop = 400
                    pplanes.stPlaneInfo[2].stMPOClipRect.lTop = 400
                    pplanes.stPlaneInfo[2].stMPODstRect.lRight = 720
                    pplanes.stPlaneInfo[2].stMPOClipRect.lRight = 720
                    pplanes.stPlaneInfo[2].stMPODstRect.lBottom = 1280
                    pplanes.stPlaneInfo[2].stMPOClipRect.lBottom = 1280
                else:
                ...
                self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Top", 3)
                self.perform_flip(pplanes)
...
if __name__ == '__main__':
    ...
    logging.info('Test purpose: Flips NV12_YUV_420 and RGB planes and checks for flickr/corruption with plane resize')
