########################################################################################################################
# @file         mpo_plane_format_media_snapmode.py
# @brief        Basic test to verify underruns during resize of 1 RGB and 1 NV12 planes in snapmode.
#               * Apply Single display configuration across all the displays.
#               * Submit flips with 3 planes where 2 planes are of RGB format and 1 plane is of NV12 format in snapmode.
...
##
# @brief    Contains function to check underruns during resize of 1 RGB and 1 NV12 planes in snapmode
class MPOPlaneFormatMediaSnapMode(mpo_base.MPOBase):
    display = None
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                rect3 = flip.MPO_RECT(0, 0, 1600, resolution.VtRes)
...
if __name__ == '__main__':
    ...
    logging.info('Test purpose: Flips NV12_YUV_420 and RGB planes checks for flickr/corruption during resize')
