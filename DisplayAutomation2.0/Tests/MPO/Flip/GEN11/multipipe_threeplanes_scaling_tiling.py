########################################################################################################################
# @file         multipipe_threeplanes_scaling_tiling.py
# @brief        This script contains test to verify flips on 3 planes on multiple displays with different sizes/tiling
...
##
# @brief    Contains test to verify flips on 3 planes on multiple displays with different sizes/tiling
class Test_ThreePlanesMPO(MPOBase):
    ...
    def runTest(self):
        ...
        for dimension1 in self.destList:
            ...
            # Y,Y,Y
            logging.debug("Tiling : Plane 1 - Y Tile ; Plane 2 - Y Tile ; Plane 3 - Y Tile")
            ...
            ##
            # On DG2 Tile Y is deprecated and linear is not possible due to GMM issues.
            # Adding a check for DG2 to skip switching the tile format. This will be removed once Tile4 tests are enabled.
            if self.platform != 'dg2':
                for index in range(0, self.NoOfDisplays):
                    for index in range(0, planes.uiPlaneCount):
                        if (index % 3 == 0):
                            planes.stPlaneInfo[
                                index].eSurfaceMemType = flip.SURFACE_MEMORY_TYPE.SURFACE_MEMORY_Y_LEGACY_TILED
                ...
                # Y,X,Y
                logging.debug("Tiling : Plane 1 - Y Tile ; Plane 2 - X Tile ; Plane 3 - Y Tile")
                self.performFlip(planes)
            ...
            if self.platform != 'dg2':
                ...
                # Linear,X,Y
                logging.debug("Tiling : Plane 1 - Linear ; Plane 2 - X Tile ; Plane 3 - Y Tile")
                self.performFlip(planes)
            ...
            for index in range(0, self.NoOfDisplays):
                ...
                # Y,Linear,Y
                logging.debug("Tiling : Plane 1 - Y Tile ; Plane 2 - Linear ; Plane 3 - Y Tile")
                self.performFlip(planes)
...
if __name__ == '__main__':
    ...
    logging.info('Test purpose: Flips 3 planes on multiple displays with different sizes/tiling'
