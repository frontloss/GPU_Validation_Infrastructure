...
#               * Submit flips with 2 planes where 2 planes are of YUV_422 format.
#                 YUV422 format represents media content and youtube playback.
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                logging.info(
                    self.mpo_helper_dft.getStepInfo() + 'Performed flip of 2 planes: YUV_422_PACKED_8_BPC, YUV_422_PACKED_8_BPC')
                ##
                ...
                if not plane1_result or not plane2_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: YUV_422_PACKED_8_BPC, YUV_422_PACKED_8_BPC",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: YUV_422_PACKED_8_BPC, YUV_422_PACKED_8_BPC')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Right", 3)):
                    self.perform_flip(pplanes)
                ...
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[0], "Right", -3) and self.mpo_helper_dft.resize(
...
if __name__ == '__main__':
    ...
    logging.info(
        'Test purpose: Flips YUV_422_PACKED_8_BPC planes and checks for watermark/underrun errors for resized planes')
