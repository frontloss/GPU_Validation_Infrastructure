...
class MPOPlaneFormatMediaResize(mpo_base.MPOBase):
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                for display in range(len(self.connected_list)):
                    ...
                    if not plane1_result or not plane2_result:
                        gdhm.report_bug(
                            title="[MPO][Plane scaling]Plane verification failed: NV12_YUV_420, RGB_8888 on panel {0}"
                        ...
                        self.fail('Plane verification failed: NV12_YUV_420, RGB_8888 on panel %s' % self.connected_list[
                            display])
    ...
    def runTest(self):
        ...
        if self.config.set_display_configuration_ex(topology, self.connected_list) is True:
            ...
            while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Bottom", 6) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Right", 6)
                   and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3], "Bottom", 6) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3], "Right",
                                                                                        6)):
                self.perform_flip(pplanes)

            ##
            # Resize the Bottom, Right of the plane
            while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Bottom", -8) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Right",
                                                                                     -8)
                   and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3], "Bottom", -8) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3],
                                                                                         "Right", -8)):
                self.perform_flip(pplanes)

            ##
            # Resize Left side of the plane
            while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Left", -6) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3], "Left", -6)):
                self.perform_flip(pplanes)

            ##
            # Resize Left side of the plane
            while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Left", 8) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3], "Left", 8)):
                self.perform_flip(pplanes)

            ##
            # Resize Top side of the plane
            while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Top", -6) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3], "Top", -6)):
                self.perform_flip(pplanes)

            ##
            # Resize Top side of the plane
            while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Top", 8) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[3], "Top", 8)):
                self.perform_flip(pplanes)
        else:
            self.fail(
                'Failed to apply display configuration %s for displays %s' % (
                    DisplayConfigTopology(topology).name, self.connected_list))
