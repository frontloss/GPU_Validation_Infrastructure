...
# @brief        Basic test to verify underruns during resize of 1 RGB and 1 YUV planes in snapmode.
#               * Apply Single display configuration across all the displays.
#               * Submit flips with 3 planes where 2 planes are of RGB format and 1 plane is of YUV422 format in snapmode.
...
##
# @brief    Contains function to check underruns during resize of 1 RGB and 1 YUV planes in snapmode
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[2], "Right", -8)):
                    ...
                    if pplanes.stPlaneInfo[1].stMPODstRect.lLeft < pplanes.stPlaneInfo[1].stMPODstRect.lRight:
...
if __name__ == '__main__':
    ...
    logging.info(
        'Test purpose: Flips YUV_422_PACKED_8_BPC and RGB planes in snap mode and  checks for watermark/underrun errors')
