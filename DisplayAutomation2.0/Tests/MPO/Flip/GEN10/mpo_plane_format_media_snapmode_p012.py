########################################################################################################################
# @file         mpo_plane_format_media_snapmode_p012.py
# @brief        Basic test to verify underruns during resize of 1 RGB and 1 P012 YUV planes in snapmode.
#               * Apply Single display configuration across all the displays.
#               * Submit flips with 3 planes where 2 planes are of RGB format and 1 plane is of P012 format in snapmode.
#                 RGB format represents the desktop and 3D application, P012 represents media content.
...
##
# @brief    Contains function to check underruns during resize of 1 RGB and 1 P012 YUV planes in snapmode
class MPOPlaneFormatMediaSnapModeP012(mpo_base.MPOBase):
    display = None
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                logging.info(self.mpo_helper_dft.getStepInfo() + 'Performed flip of 3 planes: P012_YUV_420_12_BIT, RGB_8888, RGB_8888')
                ##
                ...
                if not plane1_result or not plane2_result or not plane3_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: P012_YUV_420_12_BIT, RGB_8888, RGB_8888",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: P012_YUV_420_12_BIT, RGB_8888, RGB_8888')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                plane3 = flip.PLANE_INFO(0, 2, 1, flip.PIXEL_FORMAT.PIXEL_FORMAT_P012YUV420,
