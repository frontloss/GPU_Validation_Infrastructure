########################################################################################################################
# @file         mpo_plane_format_up_down_scaling_p010.py
# @brief        Basic test to verify scaling of planes.
#               * Apply Single display configuration across all the displays.
#               * Submit 2 flips, where 1 plane is of P010 format and another with RGB format.
...
##
# @brief    Contains function to check up down scaling of plane in in P010 YUV format
class MPOPlaneFormatUpDownScalingP010(mpo_base.MPOBase):
    display = None
            ...
            for i in range(10):
                for plane1_src_hres, plane1_src_vres in plane_resolutions:
                    ...
                    for dst_hres, dst_vres in plane_resolutions:
                        ...
                        ##
                        # Change the tile format
                        pplanes.stPlaneInfo[0].eSurfaceMemType = flip.SURFACE_MEMORY_TYPE.SURFACE_MEMORY_Y_LEGACY_TILED
                        pplanes.stPlaneInfo[1].eSurfaceMemType = flip.SURFACE_MEMORY_TYPE.SURFACE_MEMORY_Y_LEGACY_TILED
        else:
