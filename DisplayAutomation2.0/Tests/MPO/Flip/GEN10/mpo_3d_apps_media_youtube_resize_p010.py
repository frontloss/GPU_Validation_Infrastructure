########################################################################################################################
# @file         mpo_3d_apps_media_youtube_resize_p010.py
...
#               * Submit flips with 3 planes where 2 planes are of P010 format and 1 plane is of RGB format.
#                 RGB format represents the desktop, P010 represents media content and youtube playback.
...
##
# @brief    Contains function to check during resize of plane in P010 YUV format
class MPO3DAppsMediaYoutubeResizeP010(mpo_base.MPOBase):
    display = None
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                logging.info(
                    self.mpo_helper_dft.getStepInfo() + 'Performed flip of 3 planes: P010_YUV_420_10_BIT, P010_YUV_420_10_BIT, RGB_8888')
                ##
                ...
                if not plane1_result or not plane2_result or not plane3_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: P010_YUV_420_10_BIT, P010_YUV_420_10_BIT, RGB_8888",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: P010_YUV_420_10_BIT, P010_YUV_420_10_BIT, RGB_8888')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                rect3 = flip.MPO_RECT(0, 0, resolution.HzRes // 3, resolution.VtRes // 3)
                blend_value3 = flip.MPO_BLEND_VAL(0)
                plane3 = flip.PLANE_INFO(0, 2, 1, flip.PIXEL_FORMAT.PIXEL_FORMAT_P010YUV420,
                ...
                ##
                # Resize the Right, Bottom of the plane
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Right", -4) and self.mpo_helper_dft.resize(
