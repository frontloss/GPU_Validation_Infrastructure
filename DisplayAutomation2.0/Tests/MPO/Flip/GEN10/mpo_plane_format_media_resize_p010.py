########################################################################################################################
# @file         mpo_plane_format_media_resize_p010.py
# @brief        Basic test to verify underruns during resize of plane in P010 YUV format.
#               * Apply Single display configuration across all the displays.
#               * Submit flips with 2 planes where 1 plane is of P010 format and 1 plane is of RGB format.
#                 RGB format represents the desktop and P010 represents media content.
...
##
# @brief    Contains function to check underruns during resize of plane in P010 YUV format
class MPOPlaneFormatMediaResizeP010(mpo_base.MPOBase):
    display = None
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                logging.info(self.mpo_helper_dft.getStepInfo() + 'Performed flip of 2 planes: P010_YUV_420_10_BIT, RGB_8888')
                ##
                ...
                if not plane1_result or not plane2_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: P010_YUV_420_10_BIT, RGB_8888",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: P010_YUV_420_10_BIT, RGB_8888')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                plane2 = flip.PLANE_INFO(0, 1, 1, flip.PIXEL_FORMAT.PIXEL_FORMAT_P010YUV420,
                ...
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Bottom", 4) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Right",
                                                                                        4)):
                    self.perform_flip(pplanes)

                pplanes.stPlaneInfo[1].stMPOSrcRect.lLeft = resolution.HzRes // 2
                pplanes.stPlaneInfo[1].stMPODstRect.lLeft = resolution.HzRes // 2
                pplanes.stPlaneInfo[1].stMPOClipRect.lLeft = resolution.HzRes // 2
                pplanes.stPlaneInfo[1].stMPOSrcRect.lTop = resolution.VtRes // 2
                pplanes.stPlaneInfo[1].stMPODstRect.lTop = resolution.VtRes // 2
                pplanes.stPlaneInfo[1].stMPOClipRect.lTop = resolution.VtRes // 2
                pplanes.stPlaneInfo[1].stMPOSrcRect.lRight = resolution.HzRes
                pplanes.stPlaneInfo[1].stMPODstRect.lRight = resolution.HzRes
                pplanes.stPlaneInfo[1].stMPOClipRect.lRight = resolution.HzRes
                pplanes.stPlaneInfo[1].stMPOSrcRect.lBottom = resolution.VtRes
                pplanes.stPlaneInfo[1].stMPODstRect.lBottom = resolution.VtRes
                pplanes.stPlaneInfo[1].stMPOClipRect.lBottom = resolution.VtRes
                ...
                self.fail(
                    "Failed to apply display configuration %s for display %s" % (
                        DisplayConfigTopology(topology).name, self.connected_list[display_index]))


if __name__ == '__main__':
    TestEnvironment.initialize()
    logging.info("Test purpose: Flips RGB and P010 YUV planes and checks for flicker/corruption")
