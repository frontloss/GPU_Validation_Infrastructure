########################################################################################################################
# @file         mpo_plane_format_media_resize_p012.py
...
#               * Submit flips with 2 planes where 1 plane is of P012 format and 1 plane is of RGB format.
#                 RGB format represents the desktop and P012 represents media content.
...
class MPOPlaneFormatMediaResizeP012(mpo_base.MPOBase):
    display = None
    ...
    def perform_flip(self, pplanes):
        ...
        if checkmpo_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
            ...
            if ssa_result == flip.PLANES_ERROR_CODE.PLANES_SUCCESS:
                ...
                logging.info(self.mpo_helper_dft.getStepInfo() + 'Performed flip of 2 planes: P012_YUV_420_12_BIT, RGB_8888')
                ##
                ...
                if not plane1_result or not plane2_result:
                    gdhm.report_bug(
                        title="[MPO][Plane scaling]Plane verification failed: P012_YUV_420_12_BIT, RGB_8888",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E2
                    )
                    self.fail('Plane verification failed: P012_YUV_420_12_BIT, RGB_8888')
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                ##
                # Resize the Top, Left of the plane
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Left", 4) and self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Top",
                                                                                        4)):
                    self.perform_flip(pplanes)
            else:
                self.fail(
...
if __name__ == '__main__':
    ...
    logging.info("Test purpose: Flips RGB and P012 YUV planes and checks for flicker/corruption"
                 " during plane resize")
