########################################################################################################################
# @file         mpo_3d_apps_resize.py
# @brief        Basic test to verify underruns during resize of plane in RGB format.
#               * Apply Single display configuration across all the displays.
#               * Submit flips with 2 planes where 2 planes are of RGB format.
...
##
# @brief    Contains function to check underruns during resize of plane in RGB format
class MPO3DAppsResize(mpo_base.MPOBase):
    display = None
    ...
    def runTest(self):
        ...
        for display_index in range(len(self.connected_list)):
            if self.config.set_display_configuration_ex(topology, [self.connected_list[display_index]]) is True:
                ...
                pplanes.stPlaneInfo[1].stMPOSrcRect.lLeft = 400
                pplanes.stPlaneInfo[1].stMPODstRect.lLeft = 400
                pplanes.stPlaneInfo[1].stMPOClipRect.lLeft = 400
                ...
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Right", 8)):
                    ...
                    self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Right", -6)
                    self.perform_flip(pplanes)

                ##
                # Resize the left of the plane
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Left", -8)):
                    self.perform_flip(pplanes)

                    self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Left", 6)
                    self.perform_flip(pplanes)

                pplanes.stPlaneInfo[1].stMPOSrcRect.lTop = 600
                pplanes.stPlaneInfo[1].stMPODstRect.lTop = 600
                pplanes.stPlaneInfo[1].stMPOClipRect.lTop = 600

                self.perform_flip(pplanes)

                ##
                # Resize the bottom of the plane
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Bottom", 8)):
                    self.perform_flip(pplanes)

                    self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Bottom", -6)
                    self.perform_flip(pplanes)

                ##
                # Resize the top of the plane
                while (self.mpo_helper_dft.resize(pplanes.stPlaneInfo[1], "Top", -8)):
                    self.perform_flip(pplanes)
...
if __name__ == '__main__':
    ...
    logging.info('Test purpose: Flips RGB planes and  checks for watermark/underrun errors')
