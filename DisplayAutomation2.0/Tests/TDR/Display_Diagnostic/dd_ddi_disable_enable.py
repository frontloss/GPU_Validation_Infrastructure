##
# @file  dd_ddi_disable_enable.py
# @brief To verify the driver is able to give display diagnostics information to OS after driver disable-enable
...
#        driver restart(disable/enable)
class DisplayDiagnosticDisableEnable(DisplayDiagnosticBase):

    ##
    # @brief runTest - Apply display configuration based on command lines and run display diagnostic tool
    #                  Verify Diagnostic details, perform driver disable/enable and verify diagnostic details
    # @return None
    def runTest(self):
        ...
        logging.debug("Running Display Diagnostics Tool before Driver Restart")
        self.run_DDI_test_tool()

        logging.info("---------- Display Diagnostics verification ----------")
        if not self.verify_Display_Diagnostics():
            self.fail("FAIL: Display Diagnostics verification failed before Driver Restart")
        logging.info("PASS: Display Diagnostics verification passed before Driver Restart")

        logging.info("Restarting Display Driver")
        status, reboot_required = display_essential.restart_gfx_driver()
        logging.info("Restarting Display Driver Successful")

        # Run tool to test Display Diagnostic
        logging.debug("Running Display Diagnostics Tool after Driver Restart")
        self.run_DDI_test_tool()

        logging.info("---------- Display Diagnostics verification ----------")
        if not self.verify_Display_Diagnostics():
            self.fail("FAIL: Display Diagnostics verification failed after Driver Restart")
        logging.info("PASS: Display Diagnostics verification passed after Driver Restart")
