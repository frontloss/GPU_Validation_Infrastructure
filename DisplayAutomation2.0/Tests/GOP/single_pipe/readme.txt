...
2. get_tenh_offset.nsh ->  Got the 10h offset in realtime dumped to  tenh_offset.txt file in UEFI shell using the
   command "mm -w 8 -n -pci 20010 > tenh_offset.txt".
	...
	single_mmio_offsets.json -> which has required offsets of eDP+HDMI+DP

3. generate_mmio_comands_dump.py ->
	a. Changed the last bit of the 10h offest that we dumped to the tenh_offset.txt file to '0'. This gives the base
	   address.
	b. After getting the base address, we are adding each offset from the single_mmio_offsets.json to the base address and the
...
5. map_mmio_values_and_offsets_into_json.py -> Using the above generated mmio_raw_dump.txt and single_mmio_offsets.json files, we
...
1. single_pipe_Pipe_verification.py -> Verifying the Expected value of a MMIO offset (related to Pipe) taken from the
                                       ...
                                       single_pipe_pipe_verification.log file.

2. single_pipe_Port_verification.py -> Verifying the Expected value of a MMIO offset (related to Plane) taken from the
                                       mmio_dump.json(in Pre_boot) with the Actual output and making the test Pass or
                                       Fail based on the verification results. The Verification results are dumped into
                                       single_pipe_plane_verification.log file.

3. single_pipe_Port_verification.py -> Verifying the Expected value of a MMIO offset (related to Port) taken from the
                                       mmio_dump.json(in Pre_boot) with the Actual output and making the test Pass or
                                       Fail based on the verification results. The Verification results are dumped into
                                       single_pipe_port_verification.log file.

4. single_pipe_Transcoder_verification.py -> Verifying the Expected value of a MMIO offset (related to Transcoder) taken
                                             ...
                                             results are dumped into single_pipe_transcoder_verification.log file.
...
post_boot.py -> This will take port as command line argument in case of DP/ HDMI. But for eDP case default argument is
                ...
                For single eDP : post_boot.py
                For single HDMI/ DP: post_boot.py -<port>
                Port mapping for DP:
                    TCP0: USBC0
                    TCP1: USBC0
                    TCP2: USBC0
