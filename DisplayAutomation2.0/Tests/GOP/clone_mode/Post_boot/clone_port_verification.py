########################################################################################################################
# @file         clone_port_verification.py
...
#               verification results. The Verification results are dumped into clone_Port_Verification.log file.
...
logging.basicConfig(
    ...
    filename='clone_Port_Verification.log',
    filemode='w'
    )

if len(sys.argv) < 3:
    logging.info("Expected two ports as arguments")
    sys.exit(1)
...
def to_verify_actual_and_expected_plane(final_required_json):
    ...
    for ofst in mmio_dump_data:      
        ...
        # Verify offset 0x16FA00, 0x16F200
        if ((ofst['MMIO Offset'] in ['0x16FA00', '0x16F200', '0x16F400', '0x16F600', '0x16F800'])
                and (ofst['MMIO Offset'] in port_offset_dict[port_1]
                     or ofst['MMIO Offset'] in port_offset_dict[port_2])):
            ...
            if ofst['Expected Output'] is not None:
                ...
                # Check bits 3-1: Port Width (Need to check and write logic for other 2 types of eDP and HDMI/DP).
                ...
                if actual_mmio_value[12:14] == '00':
                    logging.info(logger_template_pass.format(res="PASS",
                                                             ofst="{} - {}".format(ofst['MMIO Offset'],
                                                                                   ofst['Function Name']),
                                                             verify="PHY data lane width is 00 for eDP, HDMI in TMDS "
                                                                    "and DP in 8b/10b"))
                else:
                    logging.error(logger_template_fail.format(res="FAIL",
                                                              ofst="{} - {}".format(ofst['MMIO Offset'],
                                                              ...
                                                              verify="PHY data lane width should be set to 00 for eDP, "
                                                                     "HDMI in TMDS and DP in 8b/10b (bits: 19-18).",
                                                              act=ofst['Expected Output']))
        ...
        # Verify offset 0x16FA04, 0x16F204
        elif ((ofst['MMIO Offset'] in ['0x16FA04', '0x16F204', '0x16F404', '0x16F604', '0x16F804'])
              and (ofst['MMIO Offset'] in port_offset_dict[port_1] or ofst['MMIO Offset'] in port_offset_dict[port_2])):
            logging.info(f'-----Verifying {ofst["Function Name"]} -----')
            if ofst['Expected Output'] is not None:
                actual_mmio_value = convert_hex_to_32bit_binary(ofst['Expected Output'])
        ...
        # Verify offset 0x16FA08, 0x16F208
        ...
        # Verify offset 0x64000, 0x64100, 0x64300
            ...
            if ofst['Expected Output'] is not None:
                ...
                # Check bits 3-1: Port Width (Need to check and write logic for other 2 types of eDP, DP, HDMI).
                if actual_mmio_value[28:31] == '011':
                    logging.info(logger_template_pass.format(res="PASS",
                                                             ofst="{} - {}".format(ofst['MMIO Offset'],
                                                                                   ofst['Function Name']),
                ...
                if actual_mmio_value[15] == '0':
                    ...
                    logging.error(logger_template_fail.format(res="FAIL",
                                                              ofst="{} - {}".format(ofst['MMIO Offset'],
                                                                                    ofst['Function Name']),
                                                              verify="Port Reversal should not be True (bits: 16).",
                                                              act=ofst['Expected Output']))
        ...
        # Verify offset 0x16FA10, 0x16F210
        ...
        # Verify offset 0x16FA60, 0x16F260
