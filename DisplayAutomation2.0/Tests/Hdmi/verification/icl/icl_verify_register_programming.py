#####################################################################################################################################
# @file         icl_verify_register_programming.py
# @brief        Verifies Register Programming for the ICL Platform
# @details
#               ref: icl_verify_register_programming.py \n
...
##
# @brief        IclVerifyRegisterProgramming Class
class IclVerifyRegisterProgramming:
    ddi_select_map = dict([
        (0b000, 'None'),
        (0b001, 'B'),
        (0b010, 'C'),
    ...
    ##
    # @brief        Verigy Video dip gcp
    # @return       result - Video dip gcp result
    ...
    def verify_trans_ddi_func_ctl(self):
        ...
        if self.edid_parameters.display_type in self.DISPLAY_TYPE_ARRAY:
            # If "HdmiNoNullPacketAndAudio" is set, assign DVI mode. Else, HDMI mode of operation remains unchanged
            if read_val == 1 and 'HDMI' in self.test_parameters.display_port:
                self.edid_parameters.display_type = self.DISPLAY_TYPE_ARRAY[1]
        ...
        if (display_port == "DP_A" or display_port == "EDP_A" or display_port == "MIPI_A" or display_port == "MIPI_B"):
            ...
            if (transcoder_enable == trans_conf_register.__getattribute__("transcoder_enable_ENABLE")
                    and trans_ddi_function_enable == trans_ddi_func_ctl_register.__getattribute__(
                        "trans_ddi_function_enable_ENABLE")):
                edp_dsi_input_select = trans_ddi_func_ctl_obj.edp_dsi_input_select  # (Bits Range 12:14)
...
if __name__ == "__main__":
    ...
    result, return_params = IclVerifyRegisterProgramming(test_parameters,
