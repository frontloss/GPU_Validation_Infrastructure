########################################################################################################################
# @file         verify_register_programming.py
# @brief        This functions verifies register
# @details      ref:          verify_register_programming.py \n
...
##
# @brief        VerifyReisgterProgramming
class VerifyRegisterProgramming:
    ddi_select_map = dict([
        (0b0001, 'A'),
        (0b0010, 'B'),
        (0b0011, 'C'),
    ...
    edp_dsi_input_select_map = dict([
        (0b000, 'A'),
        (0b101, 'B'),
        (0b110, 'C'),
        (0b111, 'D')
    ])
    display_port_to_usbc_map = dict([
        ('HDMI_D', 'HDMI_USBC1'),
        ('HDMI_E', 'HDMI_USBC2'),
        ('HDMI_F', 'HDMI_USBC3'),
        ('HDMI_G', 'HDMI_USBC4'),
    ...
    DDI_SUFFIX_ARRAY = ['A', 'B', 'C', 'USBC1', 'USBC2', 'USBC3', 'USBC4', 'USBC5', 'USBC6']
    ...
    ##
    # @brief        Constructor
    # @param[in]    test_parameters - Test Parameter Values
    # @param[in]    edid_parameters - EDID Parameters
    def __init__(self, test_parameters, edid_parameters):
        self.test_parameters = test_parameters
        self.edid_parameters = edid_parameters
        self.platform = test_parameters.platform.lower()
    ...
    ##
    # @brief        Verify_plane_ctl_1 Class
    # @return       result - Plane verification result
    ...
    ##
    # @brief        Verify video dip Control
    # @return       result
    def verify_video_dip_ctl(self):
        ...
        # If Transcoder is enabled in non HDMI mode, then expected_vdip_enable_avi should be 0
        expected_vdip_enable_avi = 1 if \
            (trans_ddi_func_ctl_reg.trans_ddi_mode_select ==
             getattr(trans_ddi_func_ctl_reg_module, 'trans_ddi_mode_select_HDMI')) else 0
    ...
    def verify_pipe_misc(self):
        ...
        ##
        # Check hdr_mode is disabled , as in this test case we are not enabling hdr
        # hdr_mode = hdmi_helper.get_register_value_by_range(pipe_misc_reg.asUint, 23, 23, pipe_misc_reg_offset_name + '_' + "hdr_mode")
        hdr_mode = pipe_misc_reg.hdr_mode
        expected_hdr_mode = pipe_misc_reg_module.__getattribute__("hdr_mode_DISABLE")
        feature_name = pipe_misc_reg_offset_name + '_' + "hdr_mode"
        result &= validate_feature(hdr_mode, expected_hdr_mode, feature_name)
    ...
    ##
    # @brief        Get enabled Pipi transcoder DDI Suffix
    # @return       (bool,str) - (result,out_params)
    def get_enabled_pipe_transcoder_ddi_suffix(self):
        '''
            Get pipe, transcoder, ddi suffix enabled for display
        '''
        logging.debug("****************************************************************************")
        trans_suffix_offsets = ['A', 'B', 'C', 'D']
        result = False
        ddi_suffix = None
        pipe_suffix = None
        trans_suffix = None

        if self.test_parameters.display_port in self.display_port_to_usbc_map.keys():
            display_port = self.display_port_to_usbc_map.get(self.test_parameters.display_port)
        else:
        ...
        if display_port == "DP_A" or display_port == "EDP_A" or display_port == "MIPI_A" or display_port == "MIPI_B":
            ...
            ##
            # TRANS_CONF
            trans_conf_reg_offset_name = 'TRANS_CONF' + '_' + trans_suffix
            trans_conf_reg = MMIORegister.read("TRANS_CONF_REGISTER", trans_conf_reg_offset_name, self.platform)
            transcoder_enable = trans_conf_reg.transcoder_enable  # (Bits Range 31:31)

            ##
            # TRANS_DDI_FUNC_CTL
            trans_ddi_func_ctl_reg_offset_name = 'TRANS_DDI_FUNC_CTL' + '_' + self.trans_suffix
            trans_ddi_func_ctl_reg = MMIORegister.read("TRANS_DDI_FUNC_CTL_REGISTER",
                                                       trans_ddi_func_ctl_reg_offset_name, self.platform)
            trans_ddi_function_enable = trans_ddi_func_ctl_reg.trans_ddi_function_enable  # (Bits Range 31:31)
            if (transcoder_enable == trans_conf_reg_module.__getattribute__("transcoder_enable_ENABLE")
                    and trans_ddi_function_enable == trans_ddi_func_ctl_reg_module.__getattribute__(
                        "trans_ddi_function_enable_ENABLE")):
                edp_dsi_input_select = trans_ddi_func_ctl_reg.edp_dsi_input_select  # (Bits Range 12:14)
                pipe_suffix = self.edp_dsi_input_select_map.get(edp_dsi_input_select)
            ...
            for suffix in trans_suffix_offsets:
                ##
                # TRANS_CONF
                trans_conf_reg_offset_name = 'TRANS_CONF' + '_' + suffix
                trans_conf_reg = MMIORegister.read("TRANS_CONF_REGISTER", trans_conf_reg_offset_name, self.platform)
                transcoder_enable = trans_conf_reg.transcoder_enable  # (Bits Range 31:31)

                ##
                # TRANS_DDI_FUNC_CTL
                trans_ddi_func_ctl_reg_offset_name = 'TRANS_DDI_FUNC_CTL' + '_' + suffix
                trans_ddi_func_ctl_reg = MMIORegister.read("TRANS_DDI_FUNC_CTL_REGISTER",
                ...
                if (transcoder_enable == trans_conf_reg_module.__getattribute__("transcoder_enable_ENABLE")
                        and trans_ddi_function_enable == trans_ddi_func_ctl_reg_module.__getattribute__(
                            "trans_ddi_function_enable_ENABLE")):
                    ddi_select = trans_ddi_func_ctl_reg.ddi_select
                    logging.info("ddi_select = %s" % ddi_select)
...
if __name__ == "__main__":
    ...
    test_parameters.display_port = "HDMI_D"
    edid_parameters = HdmiEDIDInfo()
    result, return_params = VerifyRegisterProgramming(test_parameters,
