########################################################################################################################
# @file         Gen13DisplayRegsInterface.py
# @brief        Contains DisplayRegsService implementation for Gen13 platforms
#
# @author       Rohit Kumar
########################################################################################################################

from DisplayRegs.DisplayRegsInterface import *
from DisplayRegs.Gen13.Pipe.Gen13PipeRegs import *
from DisplayRegs.Gen13.Plane.Gen13PlaneRegs import *
from DisplayRegs.Gen13.Transcoder.Gen13TranscoderRegs import *
from DisplayRegs.Gen13.DisplayPowerHandler.Gen13DisplayPowerRegs import *
from DisplayRegs.Gen13.Interrupt.Gen13InterruptRegs import *


class Gen13DisplayRegsService(DisplayRegsService):
    ...
    def get_trans_ddi_offsets(self, transcoder: str) -> TransDDiOffsets:
        transcoder_name = transcoder.split('_')[1]
        return TransDDiOffsets(
            getattr(OFFSET_TRANS_DDI_FUNC_CTL, f'TRANS_DDI_FUNC_CTL_{transcoder_name}'),
        )
    ...
    def get_dc6v_offsets(self) -> Dc6vOffsets:
        return Dc6vOffsets(
            getattr(OFFSET_SCANLINE_GB1, 'SCANLINE_GB1'),
    ...
    ##
    # @brief        Exposed API to get CMTG related offsets in Gen13.1
    # @param[in]    transcoder, TranscoderType
    # @return       CmtgOffsets
    def get_cmtg_offsets(self, transcoder: TranscoderType) -> CmtgOffsets:
        transcoder_name = transcoder.name.split('_')[1]
        return CmtgOffsets(
            getattr(OFFSET_TRANS_CMTG_CTL, 'TRANS_CMTG_CTL_CMTG'),
            getattr(OFFSET_TRANS_HTOTAL, 'TRANS_HTOTAL_CMTG'),
            getattr(OFFSET_TRANS_HBLANK, 'TRANS_HBLANK_CMTG'),
            getattr(OFFSET_TRANS_HSYNC, 'TRANS_HSYNC_CMTG'),
            getattr(OFFSET_TRANS_VTOTAL, 'TRANS_VTOTAL_CMTG'),
            getattr(OFFSET_TRANS_VBLANK, 'TRANS_VBLANK_CMTG'),
            getattr(OFFSET_TRANS_VSYNC, 'TRANS_VSYNC_CMTG'),
            getattr(OFFSET_LINKM, 'TRANS_LINKM1_CMTG'),
            getattr(OFFSET_LINKN, 'TRANS_LINKN1_CMTG'),
    ...
    def get_cmtg_info(self, transcoder: TranscoderType, data: CmtgOffsetValues = None) -> CmtgInfo:
        ...
        return CmtgInfo(
            ...
            h_blank.HorizontalBlankStart,h_blank.HorizontalBlankEnd, h_sync.HorizontalSyncStart,h_sync.HorizontalSyncEnd,
            v_total.VerticalTotal,v_total.VerticalActive,v_blank.VerticalBlankStart,v_blank.VerticalBlankEnd,
            v_sync.VerticalSyncStart,v_sync.VerticalSyncEnd,link_m.LinkMValue,link_n.LinkNValue,
            context_latency.ContextLatency
        )
    ...
    def get_vrr_info(self, transcoder: TranscoderType, data: VrrOffsetValues = None) -> VrrInfo:
        ...
        return VrrInfo(
            ...
            scanline_upper.ScanLineUpper, 0, 0
        )
    ...
    def get_dc6v_info(self, data: Dc6vOffsetValues = None) -> Dc6vInfo:
        ...
        if data is not None:
            ...
            restore_time = REG_DC6V_RESTORE_TIME(regs.RestoreProgrammingTime, read_register(regs.RestoreProgrammingTime, self.gfx_index))
    ...
    def get_colorctl_info(self, pipe: str, data: ColorCtlOffsetsValues) -> ColorCtlInfo:
        ...
        return ColorCtlInfo(pipe_misc_value.PipeOutputColorSpaceSelect,
                            ...
                            , pipe_misc2_value.Yuv422Mode, csc_reg_value.PipeOutputCscEnable, csc_reg_value.PipeCscEnable,
                            csc_reg_value.PipeCscCC2Enable,
    ...
    def get_interrupt_info(self, data : InterruptOffsetValues = None) -> InterruptInfo:
        ...
        if data is not None:
            isr_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptISR, data.InterruptISR)
            imr_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptIMR, data.InterruptIMR)
            iir_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptIIR, data.InterruptIIR)
            ier_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptIER, data.InterruptIER)
        else:
            isr_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptISR, read_register(regs.InterruptISR, self.gfx_index))
            imr_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptIMR, read_register(regs.InterruptIMR, self.gfx_index))
            iir_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptIIR, read_register(regs.InterruptIIR, self.gfx_index))
            ier_data = REG_DE_PORT_INTERRUPT_DEFINITION(regs.InterruptIER, read_register(regs.InterruptIER, self.gfx_index))

        return InterruptInfo(isr_data.CmtgDelayedVblank, isr_data.CmtgVblank, isr_data.CmtgVsync,
                             imr_data.CmtgDelayedVblank, imr_data.CmtgVblank, imr_data.CmtgVsync,
                             iir_data.CmtgDelayedVblank, iir_data.CmtgVblank, iir_data.CmtgVsync,
                             ier_data.CmtgDelayedVblank, ier_data.CmtgVblank, ier_data.CmtgVsync,
        )
