...
# --------------------------------------------------------------------------
#
# @file Gen13PipeRegs.py
# @brief contains Gen13PipeRegs.py related register definitions
...
class OFFSET_PIPE_ISOCREQ:
    PIPE_ISOCREQ_A = 0x70010
    PIPE_ISOCREQ_B = 0x71010
    PIPE_ISOCREQ_C = 0x72010
    PIPE_ISOCREQ_D = 0x73010


class _PIPE_ISOCREQ(ctypes.LittleEndianStructure):
    _fields_ = [
        ('Bandwidth', ctypes.c_uint32, 16),
        ('Ltr', ctypes.c_uint32, 16),
...
class REG_PIPE_ISOCREQ(ctypes.Union):
    value = 0
    offset = 0

    Bandwidth = 0  # bit 0 to 16
    Ltr = 0  # bit 16 to 32
    Delay = 0  # bit 0 to 8
    ...
    _anonymous_ = ("bitMap",)
    _fields_ = [
        ('bitMap', _PIPE_ISOCREQ),
        ('value', ctypes.c_uint32)
    ]

    def __init__(self, offset=None, value=None):
        assert not (offset is None and value is None)
        super(REG_PIPE_ISOCREQ, self).__init__()
        if offset is not None:
            self.offset = offset
        if value is not None:
            self.value = value
