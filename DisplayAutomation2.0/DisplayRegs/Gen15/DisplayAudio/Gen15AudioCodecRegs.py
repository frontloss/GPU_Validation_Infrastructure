...
# --------------------------------------------------------------------------
#
# @file Gen15AudioCodecRegs.py
# @brief contains Gen15AudioCodecRegs.py related register definitions

import ctypes
from enum import Enum


class ENUM_COLOR_DEPTH_HDMI(Enum):
    COLOR_DEPTH_HDMI_8_BPC = 0x0  # Per HDMI spec, N value lookup is based on TMDS clock = 1.00 x Pixel Clock.
    COLOR_DEPTH_HDMI_10_BPC = 0x1  # Per HDMI spec, N value lookup is based on TMDS clock = 1.25 x Pixel Clock.
    COLOR_DEPTH_HDMI_12_BPC = 0x2  # Per HDMI spec, N value lookup is based on TMDS clock = 1.50 x Pixel Clock.
    COLOR_DEPTH_HDMI_16_BPC = 0x3  # Per HDMI spec, N value lookup is based on TMDS clock = 2.00 x Pixel Clock.
...
class _AUD_CONFIG(ctypes.LittleEndianStructure):
    _fields_ = [
        ('ColorDepthHdmi', ctypes.c_uint32, 3),
...
class REG_AUD_CONFIG(ctypes.Union):
    ...
    ColorDepthHdmi = 0  # bit 0 to 2
    DisableNcts = 0  # bit 3 to 3
    LowerNValue = 0  # bit 4 to 15
    PixelClockHdmi = 0  # bit 16 to 19
    UpperNValue = 0  # bit 20 to 27
    NProgrammingEnable = 0  # bit 28 to 28
    NValueIndex = 0  # bit 29 to 29
...
class REG_AUD_CONFIG_2(ctypes.Union):
    ...
    UpperBitsForMctsValue = 0  # bit 0 to 3
    Reserved4 = 0  # bit 4 to 7
    UpperBitsForNValue = 0  # bit 8 to 11
    Reserved12 = 0  # bit 12 to 15
    Dpspecversion = 0  # bit 16 to 20
    Reserved21 = 0  # bit 21 to 30
    DisableHblankOverflowFix = 0  # bit 31 to 31
...
class REG_AUD_MISC_CTRL(ctypes.Union):
    ...
    ProAllowed = 0  # bit 1 to 1
    SampleFabricationEnBit = 0  # bit 2 to 2
    Reserved3 = 0  # bit 3 to 3
    OutputDelay = 0  # bit 4 to 7
    SamplePresentDisable = 0  # bit 8 to 8
...
class REG_AUD_PIN_ELD_CP_VLD(ctypes.Union):
    ...
    EldValida = 0  # bit 0 to 0
    CpReadya = 0  # bit 1 to 1
    AudioOutputEnablea = 0  # bit 2 to 2
    AudioInactivea = 0  # bit 3 to 3
    EldValidb = 0  # bit 4 to 4
    CpReadyb = 0  # bit 5 to 5
    AudioOutputEnableb = 0  # bit 6 to 6
    AudioInactiveb = 0  # bit 7 to 7
    EldValidc = 0  # bit 8 to 8
    CpReadyc = 0  # bit 9 to 9
    AudioOutputEnablec = 0  # bit 10 to 10
    AudioInactivec = 0  # bit 11 to 11
    EldValidd = 0  # bit 12 to 12
    CpReadyd = 0  # bit 13 to 13
    AudioOutputEnabled = 0  # bit 14 to 14
    AudioInactived = 0  # bit 15 to 15
...
class _AUD_CHICKENBIT_REG(ctypes.LittleEndianStructure):
    _fields_ = [
        ...
        ('Chicken1Spare11', ctypes.c_uint32, 1),
        ('Chicken1Spare12', ctypes.c_uint32, 1),
        ('CodecSleepOverwriteToDacfeunit', ctypes.c_uint32, 1),
        ...
        ('EnableDp1_2Features', ctypes.c_uint32, 1),
    ]
...
class REG_AUD_CHICKENBIT_REG(ctypes.Union):
    ...
    EnableMmioProgramming = 0  # bit 0 to 0
    AudioTimestampTestMode = 0  # bit 1 to 1
    EpssDisable = 0  # bit 2 to 2
    Fabrication3244Disable = 0  # bit 3 to 3
    PatternGen2ChEn = 0  # bit 4 to 4
    PatternGen8ChEn = 0  # bit 5 to 5
    DisableTimestampFixForDphbr = 0  # bit 6 to 6
    DisableTimestampDeltaErrorFor32_44Khz = 0  # bit 7 to 7
    DisablePresenceDetectPulseTransitionWhenUnsolIsDisabled = 0  # bit 8 to 8
    DisableEldValidPulseTransitionWhenUnsolIsDisabled = 0  # bit 9 to 9
    BlockAudioDataFromReachingThePort = 0  # bit 10 to 10
    Chicken1Spare11 = 0  # bit 11 to 11
    Chicken1Spare12 = 0  # bit 12 to 12
    CodecSleepOverwriteToDacfeunit = 0  # bit 13 to 13
    CodecSleepState = 0  # bit 14 to 14
    CodecWakeOverwriteToDacfeunit = 0  # bit 15 to 15
    DpComplianceFixDisableForDacbeUnit = 0  # bit 16 to 16
    ChickenBitsForDacbeUnit = 0  # bit 17 to 17
    DisableDacfeProtocolFixForDataFlitErrorInTheEndOfFrame = 0  # bit 18 to 18
    Dp1_2DeviceIndexFixDisable = 0  # bit 19 to 19
    Enable20BitSupportForSampleRate = 0  # bit 20 to 20
    DisableBclkFrameSyncFixOnBclkCounter = 0  # bit 21 to 21
    SamplePresentEnableChickenBitForDacbeUnit = 0  # bit 22 to 22
    EccOverwriteEnableChickenBitForDacbeUnit = 0  # bit 23 to 23
    DisableTheConvSampleFifoFix = 0  # bit 24 to 24
    ChickenBits25ForDacfpUnit = 0  # bit 25 to 25
    ChickenBits26ForDacfpUnit = 0  # bit 26 to 26
    ChickenBits29To27ForDacfpUnit = 0  # bit 27 to 29
    VanillaBitEnableForThreeWidgets = 0  # bit 30 to 30
    EnableDp1_2Features = 0  # bit 31 to 31
...
class ENUM_DACFP_CDCLOCK_DELAY_TO_START_SILENT_STREAM_GENERATION(Enum):
    DACFP_CDCLOCK_DELAY_TO_START_SILENT_STREAM_GENERATION_10_CLOCKS = 0x0  # Will start silent stream generation after
                                                                            #  10 cdclocks after receiving fake frame
                                                                            # sync.
    DACFP_CDCLOCK_DELAY_TO_START_SILENT_STREAM_GENERATION_50_CLOCKS = 0x1  # Will start silent stream generation after
                                                                            #  50 cdclocks after receiving fake frame
                                                                            # sync.
    DACFP_CDCLOCK_DELAY_TO_START_SILENT_STREAM_GENERATION_500_CLOCKS = 0x2  # Will start silent stream generation afte
                                                                             # r 500 cdclocks after receiving fake
                                                                             # frame sync.
    DACFP_CDCLOCK_DELAY_TO_START_SILENT_STREAM_GENERATION_5000_CLOCKS = 0x3  # Will start silent stream generation aft
                                                                              # er 5000 cdclocks after receiving fake
                                                                              # frame sync.
...
class ENUM_TIMESTAMP_PROGRAMMABLE_DP_SPEC_VERSION_FIX(Enum):
    TIMESTAMP_PROGRAMMABLE_DP_SPEC_VERSION_FIX_ENABLE = 0x0  # Enables programmable DP spec version in AUD_CONFIG_2 reg
                                                             # ister.
    TIMESTAMP_PROGRAMMABLE_DP_SPEC_VERSION_FIX_DISABLE = 0x1  # Disables programmable DP spec version, reverts to autom
                                                              # atic selection of DP spec version value.


class ENUM_TIMESTAMP_DP_ECC_FIX(Enum):
    TIMESTAMP_DP_ECC_FIX_ENABLE = 0x0  # Enables DP timestamp ECC generation fix.
    TIMESTAMP_DP_ECC_FIX_DISABLE = 0x1  # Disables DP timestamp ECC generation fix.


class ENUM_SILENT_STREAM_DP_MODE_CHANGE_FIX(Enum):
    SILENT_STREAM_DP_MODE_CHANGE_FIX_ENABLE = 0x0  # Enables suppression of the DP mode change signal when silent strea
                                                   # m is enabled.
    SILENT_STREAM_DP_MODE_CHANGE_FIX_DISABLE = 0x1  # Disables suppression of the DP mode change signal when silent str
                                                    # eam is enabled.
...
class ENUM_LOWER_CDCLOCKS_SUPPORT_(Enum):
    LOWER_CDCLOCKS_SUPPORT_ENABLE = 0x0  # cdclock frequency with less than bclk frequency is supported.
    LOWER_CDCLOCKS_SUPPORT_DISABLE = 0x1  # cdclock frequency with less than bclk frequency is not supported


class ENUM_PORT_SELECT(Enum):
    PORT_SELECT_PORT1 = 0x0  # Port 1 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT2 = 0x1  # Port 2 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT3 = 0x2  # Port 3 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT4 = 0x3  # Port 4 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT5 = 0x4  # Port 5 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT6 = 0x5  # Port 6 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT7 = 0x6  # Port 7 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT8 = 0x7  # Port 8 is selected. Valid only in single pin mode.
    PORT_SELECT_PORT9 = 0x8  # Port 9 is selected. Valid only in single pin mode.
...
class _AUD_CHICKENBIT_REG_2(ctypes.LittleEndianStructure):
    _fields_ = [
        ...
        ('DacfpCdclockDelayToStartSilentStreamGeneration', ctypes.c_uint32, 2),
        ('DacfpDisableFakeFrameSyncGeneration', ctypes.c_uint32, 1),
        ('DacfpDisableKeepAliveD3Generation', ctypes.c_uint32, 1),
        ('DacfpIgnoreKeepAliveSetting', ctypes.c_uint32, 1),
        ('DacfpEnableKeepAliveForStreamId0', ctypes.c_uint32, 1),
        ('DacfpExtraChickens', ctypes.c_uint32, 2),
        ...
        ('TimestampProgrammableDpSpecVersionFix', ctypes.c_uint32, 1),
        ('DacbeExtraChickenBit', ctypes.c_uint32, 1),
        ('TimestampDpEccFix', ctypes.c_uint32, 1),
        ('SilentStreamDpModeChangeFix', ctypes.c_uint32, 1),
        ('Disable8TModeFix', ctypes.c_uint32, 1),
        ('DacfeExtraChickens', ctypes.c_uint32, 2),
        ('LowerCdclocksSupport', ctypes.c_uint32, 1),
...
class REG_AUD_CHICKENBIT_REG_2(ctypes.Union):
    ...
    EnableVariableRefreshRateCounter = 0  # bit 0 to 0
    EnableResponseToInvalidVerbs = 0  # bit 1 to 1
    CodecSleepStateMachineStatus = 0  # bit 2 to 4
    LinkStatus = 0  # bit 5 to 5
    DisableSyncCountErrorGeneration = 0  # bit 6 to 6
    DisableCodecWakeEventToController = 0  # bit 7 to 7
    DacfpCdclockDelayToStartSilentStreamGeneration = 0  # bit 8 to 9
    DacfpDisableFakeFrameSyncGeneration = 0  # bit 10 to 10
    DacfpDisableKeepAliveD3Generation = 0  # bit 11 to 11
    DacfpIgnoreKeepAliveSetting = 0  # bit 12 to 12
    DacfpEnableKeepAliveForStreamId0 = 0  # bit 13 to 13
    DacfpExtraChickens = 0  # bit 14 to 15
    PipeaStickyBitForVrrDebug = 0  # bit 16 to 16
    PipebStickyBitForVrrDebug = 0  # bit 17 to 17
    PipecStickyBitForVrrDebug = 0  # bit 18 to 18
    PipedStickyBitForVrrDebug = 0  # bit 19 to 19
    TimestampProgrammableDpSpecVersionFix = 0  # bit 20 to 20
    DacbeExtraChickenBit = 0  # bit 21 to 21
    TimestampDpEccFix = 0  # bit 22 to 22
    SilentStreamDpModeChangeFix = 0  # bit 23 to 23
    Disable8TModeFix = 0  # bit 24 to 24
    DacfeExtraChickens = 0  # bit 25 to 26
    LowerCdclocksSupport = 0  # bit 27 to 27
    PortSelect = 0  # bit 28 to 31
...
class REG_AUD_DIP_ELD_CTRL_ST(ctypes.Union):
    ...
    DipAccessAddress = 0  # bit 0 to 3
    EldAck = 0  # bit 4 to 4
    EldAccessAddress = 0  # bit 5 to 9
    EldBufferSize = 0  # bit 10 to 14
    Reserved15 = 0  # bit 15 to 15
    DipTransmissionFrequency = 0  # bit 16 to 17
    DipBufferIndex = 0  # bit 18 to 20
    DipTypeEnableStatus = 0  # bit 21 to 24
    Reserved25 = 0  # bit 25 to 27
    DipPortSelect = 0  # bit 28 to 31
...
class REG_AUD_FREQ_CNTRL(ctypes.Union):
    ...
    _48MhzBclk = 0  # bit 3 to 3
    _96MhzBclk = 0  # bit 4 to 4
    Reserved5 = 0  # bit 5 to 10
    DetectFrameSyncEarly = 0  # bit 11 to 12
    BypassFlop = 0  # bit 13 to 13
    TMode = 0  # bit 14 to 15
    Reserved16 = 0  # bit 16 to 31
...
class REG_AUD_CONFIG_BE(ctypes.Union):
    ...
    NumberOfSamplesPerLineForPipeA = 0  # bit 0 to 1
    DpMixerMainstreamPriorityEnableForPipeA = 0  # bit 2 to 2
    Hblank_StartCountForPipeA = 0  # bit 3 to 5
    NumberOfSamplesPerLineForPipeB = 0  # bit 6 to 7
    DpMixerMainstreamPriorityEnableForPipeB = 0  # bit 8 to 8
    Hblank_StartCountForPipeB = 0  # bit 9 to 11
    NumberOfSamplesPerLineForPipeC = 0  # bit 12 to 13
    DpMixerMainstreamPriorityEnableForPipeC = 0  # bit 14 to 14
    Hblank_StartCountForPipeC = 0  # bit 15 to 17
    NumberOfSamplesPerLineForPipeD = 0  # bit 18 to 19
    DpMixerMainstreamPriorityEnableForPipeD = 0  # bit 20 to 20
    Hblank_StartCountForPipeD = 0  # bit 21 to 23
    HblankEarlyEnableForPipea = 0  # bit 24 to 24
    HblankEarlyEnableForPipeb = 0  # bit 25 to 25
    HblankEarlyEnableForPipec = 0  # bit 26 to 26
    HblankEarlyEnableForPiped = 0  # bit 27 to 27
    DelaySampleCountLatchPipeA = 0  # bit 28 to 28
    DelaySampleCountLatchPipeB = 0  # bit 29 to 29
    DelaySampleCountLatchPipeC = 0  # bit 30 to 30
    DelaySampleCountLatchPipeD = 0  # bit 31 to 31
...
class ENUM_HBLANK_EARLY_CLOCKS(Enum):
    HBLANK_EARLY_CLOCKS_LINK_CLOCKS = 0x4


class ENUM_GUARDBAND_FOR_HBLANK_END(Enum):
    GUARDBAND_FOR_HBLANK_END_PIXEL_CLOCKS = 0x8


class ENUM_PAUSE_GENERATION_AFTER_LINE_WRAP(Enum):
    PAUSE_GENERATION_AFTER_LINE_WRAP_NO_PAUSE = 0x0
    PAUSE_GENERATION_AFTER_LINE_WRAP_1_LINE_PAUSE = 0x1
    PAUSE_GENERATION_AFTER_LINE_WRAP_2_LINE_PAUSE = 0x2
    PAUSE_GENERATION_AFTER_LINE_WRAP_3_LINE_PAUSE = 0x3


class ENUM_DISABLE_GENERATION_OF_NEW_HBLANKS(Enum):
    DISABLE_GENERATION_OF_NEW_HBLANKS_DISABLED = 0x0
    DISABLE_GENERATION_OF_NEW_HBLANKS_ENABLED = 0x1


class ENUM_ENABLE_SDP_SPLIT(Enum):
    ENABLE_SDP_SPLIT_DISABLED = 0x0
    ENABLE_SDP_SPLIT_ENABLED = 0x1


class OFFSET_AUD_DP_2DOT0_CTRL:
    AUD_TCA_DP_2DOT0_CTRL = 0x650BC
    AUD_TCB_DP_2DOT0_CTRL = 0x651BC
    AUD_TCC_DP_2DOT0_CTRL = 0x652BC
    AUD_TCD_DP_2DOT0_CTRL = 0x653BC

class _AUD_DP_2DOT0_CTRL(ctypes.LittleEndianStructure):
    _fields_ = [
        ('HblankEarlyClocks', ctypes.c_uint32, 8),
        ('GuardbandForHblankEnd', ctypes.c_uint32, 16),
        ('PauseGenerationAfterLineWrap', ctypes.c_uint32, 2),
        ('Spare26', ctypes.c_uint32, 1),
        ('DisableGenerationOfNewHblanks', ctypes.c_uint32, 1),
        ('Spare30To28', ctypes.c_uint32, 3),
        ('EnableSdpSplit', ctypes.c_uint32, 1),
    ]


class REG_AUD_DP_2DOT0_CTRL(ctypes.Union):
    value = 0
    offset = 0

    HblankEarlyClocks = 0  # bit 0 to 7
    GuardbandForHblankEnd = 0  # bit 8 to 23
    PauseGenerationAfterLineWrap = 0  # bit 24 to 25
    Spare26 = 0  # bit 26 to 26
    DisableGenerationOfNewHblanks = 0  # bit 27 to 27
    Spare30To28 = 0  # bit 28 to 30
    EnableSdpSplit = 0  # bit 31 to 31

    _anonymous_ = ("bitMap",)
    _fields_ = [
        ('bitMap', _AUD_DP_2DOT0_CTRL),
        ('value', ctypes.c_uint32)
    ]

    def __init__(self, offset=None, value=None):
        assert not (offset is None and value is None)
        super(REG_AUD_DP_2DOT0_CTRL, self).__init__()
        if offset is not None:
            self.offset = offset
        if value is not None:
            self.value = value
