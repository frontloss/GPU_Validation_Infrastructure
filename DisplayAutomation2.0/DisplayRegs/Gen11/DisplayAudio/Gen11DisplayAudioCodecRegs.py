...
# --------------------------------------------------------------------------
#
# @file Gen11DisplayAudioCodecRegs.py
# @brief contains Gen11DisplayAudioCodecRegs.py related register definitions
...
class ENUM_PORT_SELECT(Enum):
    PORT_SELECT_PORTB = 0x0  # Port B is selected. Valid only in single pin mode.
    PORT_SELECT_PORTC = 0x1  # Port C is selected. Valid only in single pin mode.
    PORT_SELECT_PORTD = 0x2  # Port D is selected. Valid only in single pin mode.
    PORT_SELECT_PORTE = 0x3  # Port E is selected. Valid only in single pin mode.
    PORT_SELECT_PORTF = 0x4  # Port F is selected. Valid only in single pin mode.
...
class _AUD_CHICKENBIT_REG_2(ctypes.LittleEndianStructure):
    _fields_ = [
        ...
        ('DacbeExtraChickens', ctypes.c_uint32, 5),
        ('DacfeExtraChickens', ctypes.c_uint32, 5),
        ('PortSelect', ctypes.c_uint32, 3),
    ]
...
class REG_AUD_CHICKENBIT_REG_2(ctypes.Union):
    ...
    DacbeExtraChickens = 0  # bit 19 to 24
    DacfeExtraChickens = 0  # bit 24 to 29
    PortSelect = 0  # bit 29 to 32
...
class ENUM_DIP_PORT_SELECT(Enum):
    ...
    DIP_PORT_SELECT_DIGITAL_PORT_D = 0x3  # Digital Port D
    DIP_PORT_SELECT_DIGITAL_PORT_E = 0x4  # Digital Port E
    DIP_PORT_SELECT_DIGITAL_PORT_F = 0x5  # Digital Port F
...
class _AUD_DIP_ELD_CTRL_ST(ctypes.LittleEndianStructure):
    _fields_ = [
        ...
        ('Reserved25', ctypes.c_uint32, 4),
        ('DipPortSelect', ctypes.c_uint32, 3),
    ]
...
class REG_AUD_DIP_ELD_CTRL_ST(ctypes.Union):
    ...
    Reserved25 = 0  # bit 25 to 29
    DipPortSelect = 0  # bit 29 to 32
...
class ENUM_TMODE(Enum):
    TMODE_4T_DEFAULT = 0x0  # 4T mode with sdi data held for 4 bit clks.
...
class ENUM_HBLANK_EARLY_ENABLE_FOR_PIPEC(Enum):
    HBLANK_EARLY_ENABLE_FOR_PIPEC_HBLANK_EARLY_DISABLE = 0x0  # The default Hblank is used and a delay of 16 tcclks(pro
                                                              # grammable bits 22) is added before the SM arcs during
                                                              # each hblank for Pipe C.
    HBLANK_EARLY_ENABLE_FOR_PIPEC_HBLANK_EARLY_ENABLE = 0x1  # The early hblank programmed by fields ‘hblank_start coun
                                                             # t for Pipe C’ will be used to trigger samplecount
                                                             # calculation for Pipe C.


class ENUM_HBLANK_EARLY_ENABLE_FOR_PIPEB(Enum):
    HBLANK_EARLY_ENABLE_FOR_PIPEB_HBLANK_EARLY_DISABLE = 0x0  # The default Hblank is used and a delay of 16 tcclks(pro
                                                              # grammable bits 23) is added before the SM arcs during
                                                              # each hblank for Pipe B
    HBLANK_EARLY_ENABLE_FOR_PIPEB_HBLANK_EARLY_ENABLE = 0x1  # The early hblank programmed by fields ‘hblank_start coun
                                                             # t for Pipe B’ will be used to trigger samplecount
                                                             # calculation for Pipe B.


class ENUM_HBLANK_EARLY_ENABLE_FOR_PIPEA(Enum):
    HBLANK_EARLY_ENABLE_FOR_PIPEA_HBLANK_EARLY_DISABLE = 0x0  # The default Hblank is used and a delay of 16 tcclks(pro
                                                              # grammable bits 24) is added before the SM arcs during
                                                              # each hblank for Pipe A
    HBLANK_EARLY_ENABLE_FOR_PIPEA_HBLANK_EARLY_ENABLE = 0x1  # The early hblank programmed by fields ‘hblank_start coun
                                                             # t for Pipe A’ will be used to trigger samplecount
...
class REG_AUD_CONFIG_BE(ctypes.Union):
    ...
    HblankEarlyEnableForPipec = 0  # bit 18 to 19
    HblankEarlyEnableForPipeb = 0  # bit 19 to 20
    HblankEarlyEnableForPipea = 0  # bit 20 to 21
    DisableHblankOverflowFix = 0  # bit 21 to 22
    DelaySampleCountLatchPipeC = 0  # bit 22 to 23
    DelaySampleCountLatchPipeB = 0  # bit 23 to 24
    DelaySampleCountLatchPipeA = 0  # bit 24 to 25
    SpareBits = 0  # bit 25 to 32
