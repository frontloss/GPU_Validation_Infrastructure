########################################################################################################################
# @file         Gen11DisplayRegsInterface.py
# @brief        Contains DisplayRegsService implementation for Gen11 platforms
#
# @author       Rohit Kumar
########################################################################################################################

from DisplayRegs.DisplayRegsInterface import *
from DisplayRegs.Gen11.Pipe.Gen11PipeRegs import *
from DisplayRegs.Gen11.Plane.Gen11PlaneRegs import *
from DisplayRegs.Gen11.Transcoder.Gen11TranscoderRegs import *
from DisplayRegs.Gen11.DisplayPowerHandler.Gen11DisplayPowerRegs import *


class Gen11DisplayRegsService(DisplayRegsService):
    ...
    ##
    # @brief        Exposed API to get pipe coeff offsets list for given pipe and plane
    # @param[in]    pipe, str -'A','B','C','D'
    ...
    def get_psr_info(self, transcoder: TranscoderType, data: PsrOffsetValues = None) -> PsrInfo:
        ...
        return PsrInfo(
            srd_ctl.SrdEnable == 1, psr2_ctl.Psr2Enable == 1, False, False, False, False, psr2_ctl.IdleFrames,
            psr2_status.Psr2State == 0x0
        )
    ...
    def get_dcstate_info(self, data: DCStateOffsetsValues = None) -> DcStateInfo:
        ...
        return DcStateInfo(dc_state_en.Dc9Allow, dc_state_en.DynamicDcStateEnable == 2,
                           dc_state_en.DynamicDcStateEnable == 1, 0, 0, 0, 0)
