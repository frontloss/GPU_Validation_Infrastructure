########################################################################################################################
# @file         Gen14DisplayRegsInterface.py
# @brief        Contains DisplayRegsService implementation for Gen14 platforms
#
# @author       Rohit Kumar
########################################################################################################################

from DisplayRegs.DisplayRegsInterface import *
from DisplayRegs.Gen14.Pipe.Gen14PipeRegs import *
from DisplayRegs.Gen14.Plane.Gen14PlaneRegs import *
from DisplayRegs.Gen14.Transcoder.Gen14TranscoderRegs import *
from DisplayRegs.Gen14.DisplayPowerHandler.Gen14DisplayPowerRegs import *
from DisplayRegs.Gen14.Interrupt.Gen14InterruptRegs import *
from DisplayRegs.Gen14.DisplayAudio.Gen14AudioCodecRegs import *


class Gen14DisplayRegsService(DisplayRegsService):
    ...
    ##
    # @brief        Exposed API to get CMTG related offsets in Gen14
    # @param[in]    transcoder, TranscoderType
    # @return       CmtgOffsets
    def get_cmtg_offsets(self, transcoder: TranscoderType) -> CmtgOffsets:
        ...
        return CmtgOffsets(
            ...
            getattr(OFFSET_TRANS_SET_CONTEXT_LATENCY, f'TRANS_SET_CONTEXT_LATENCY_{transcoder_name}'),
        )
    ...
    def get_dcstate_info(self, data: DCStateOffsetsValues = None) -> DcStateInfo:
        ...
        return DcStateInfo(dc_state_en.Dc9Allow, dc_state_en.DynamicDcStateEnable == 2,
                           ...
                           dc_state_en.DisplayClockOffEnableEdp, dc_state_en.PhyClkreqPg1Latch, dc_state_en.PhyPg1Latch)
    ...
    def get_audio_ctl_offsets(self, pipe: PipeType) -> AudDP2CtlOffsets:
        ...
        return AudDP2CtlOffsets(
            getattr(OFFSET_AUD_DP_2DOT0_CTRL, f'AUD_TC_{pipe_name}_DP_2DOT0_CTRL')
        )
