###############################################################################
# Script Name         : write_dpcd
# Script Owner        : Chandrakanth Pabolu
# Description         : Helper functions to write DPCD registers for eDP & DP
###############################################################################
...
def write_dpcd_value(dpcd_offset: int, disp_target_id: int, display_name: str, dpcd_data: int) -> None:
    try:
        flag = driver_escape.write_dpcd(disp_target_id, dpcd_offset, [dpcd_data])
        if flag is True:
            output = [i for i in format(dpcd_data, "08b")]

            print("\nDPCD Write for Display: {0} with Offset: {1} = {2} is successful.".format(
                display_name, hex(dpcd_offset), hex(dpcd_data)))
        else:
            print("Unable to write DPCD Data for Display: {0} with Offset: {1} = {2}.".format(
                display_name, hex(dpcd_offset), hex(dpcd_data)))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4:
        ...
        try:
            dpcd_data = int(sys.argv[3], 16)
        except ValueError:
            print("Invalid dpcd data.")
        ...
        if display == "EDP":
            ...
            write_dpcd_value(offset, target_id, display, dpcd_data)
        else:
            for port, target_id in port_targetid_dict.items():
                if (port is not _EDP_PORT_NAME) and ("DP" in port):
                    write_dpcd_value(offset, target_id, port, dpcd_data)
        dll_logger.cleanup()
    else:
        print("Invalid arguments. Refer below for using the functionality")
        print("Usage: write_dpcd.py <Display_name> <offset_value> <dpcd_value>")
