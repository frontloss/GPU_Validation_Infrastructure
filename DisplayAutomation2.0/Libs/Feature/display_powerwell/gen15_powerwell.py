######################################################################################
# @file         gen15_powerwell.py
# @brief        Gen15 specific module for Display Power well verification
# @author       Veena, Veluru
######################################################################################
import ctypes
import logging
...
from DisplayRegs.Gen15.DisplayPowerHandler import Gen15DisplayPowerRegs
...
# otherwise.
ENABLED_POWER_WELL_MASK_LNL = 0x1E0003C63

##
# @brief        C10_PHY_POWERDOWN_STATE for Gen15 platforms
...
##
# @brief        DisplayPowerWell for Gen15 platforms
class DisplayPowerWell(ctypes.Union):
    ...
    def __init__(self, mask=None, platform=None, gfx_index='gfx_0'):
        ...
        offset = Gen15DisplayPowerRegs.OFFSET_PWR_WELL_CTL.PWR_WELL_CTL2
        value = DisplayArgs.read_register(offset, gfx_index)
        powerwell_ctl = Gen15DisplayPowerRegs.REG_PWR_WELL_CTL(offset, value)
        ...
        offset = Gen15DdiRegs.OFFSET_PORT_AUX_CTL.PORT_AUX_CTL_A
        value = DisplayArgs.read_register(offset, gfx_index)
        port_aux_ctl = Gen15DdiRegs.REG_PORT_AUX_CTL(offset, value)
        self.PowerwellAuxAIo = port_aux_ctl.PhyPowerRequest

        offset = Gen15DdiRegs.OFFSET_PORT_AUX_CTL.PORT_AUX_CTL_B
        value = DisplayArgs.read_register(offset, gfx_index)
        port_aux_ctl = Gen15DdiRegs.REG_PORT_AUX_CTL(offset, value)
        self.PowerwellAuxBIo = port_aux_ctl.PhyPowerRequest

        offset = Gen15DdiRegs.OFFSET_PORT_AUX_CTL.PORT_AUX_CTL_USBC1
        value = DisplayArgs.read_register(offset, gfx_index)
        port_aux_ctl = Gen15DdiRegs.REG_PORT_AUX_CTL(offset, value)
        self.PowerwellAuxFIo = port_aux_ctl.PhyPowerRequest

        offset = Gen15DdiRegs.OFFSET_PORT_AUX_CTL.PORT_AUX_CTL_USBC2
        value = DisplayArgs.read_register(offset, gfx_index)
        port_aux_ctl = Gen15DdiRegs.REG_PORT_AUX_CTL(offset, value)
        self.PowerwellAuxGIo = port_aux_ctl.PhyPowerRequest

        offset = Gen15DdiRegs.OFFSET_PORT_AUX_CTL.PORT_AUX_CTL_USBC3
        value = DisplayArgs.read_register(offset, gfx_index)
        port_aux_ctl = Gen15DdiRegs.REG_PORT_AUX_CTL(offset, value)
        self.PowerwellAuxHIo = port_aux_ctl.PhyPowerRequest

        offset = Gen15DdiRegs.OFFSET_PORT_AUX_CTL.PORT_AUX_CTL_USBC4
        value = DisplayArgs.read_register(offset, gfx_index)
        port_aux_ctl = Gen15DdiRegs.REG_PORT_AUX_CTL(offset, value)
        self.PowerwellAuxIIo = port_aux_ctl.PhyPowerRequest

        def get_ddi_io(port_buf_ctl_reg: Gen15DdiRegs.REG_PORT_BUF_CTL2):
            # No DDI powerwell in LNL. It is controlled be mainlink IO Power. Need to revisit in post-si
            if platform == 'LNL' or platform == 'PTL' or platform == 'NVL' or platform == 'CLS':
                return 0
            state = None
        ...
        # DDI powerwell verification is not direct in LNL. There is no DDI Powerwell in LNL,
        ...
        offset = Gen15DdiRegs.OFFSET_PORT_BUF_CTL2.PORT_BUF_CTL2_A
        value = DisplayArgs.read_register(offset, gfx_index)
        port_buf_ctl = Gen15DdiRegs.REG_PORT_BUF_CTL2(offset, value)
        self.PowerwellDdiAIo = get_ddi_io(port_buf_ctl)

        offset = Gen15DdiRegs.OFFSET_PORT_BUF_CTL2.PORT_BUF_CTL2_B
        value = DisplayArgs.read_register(offset, gfx_index)
        port_buf_ctl = Gen15DdiRegs.REG_PORT_BUF_CTL2(offset, value)
        self.PowerwellDdiBIo = get_ddi_io(port_buf_ctl)

        offset = Gen15DdiRegs.OFFSET_PORT_BUF_CTL2.PORT_BUF_CTL2_USBC1
        value = DisplayArgs.read_register(offset, gfx_index)
        port_buf_ctl = Gen15DdiRegs.REG_PORT_BUF_CTL2(offset, value)
        self.PowerwellDdiFIo = get_ddi_io(port_buf_ctl)

        offset = Gen15DdiRegs.OFFSET_PORT_BUF_CTL2.PORT_BUF_CTL2_USBC2
        value = DisplayArgs.read_register(offset, gfx_index)
        port_buf_ctl = Gen15DdiRegs.REG_PORT_BUF_CTL2(offset, value)
        self.PowerwellDdiGIo = get_ddi_io(port_buf_ctl)

        offset = Gen15DdiRegs.OFFSET_PORT_BUF_CTL2.PORT_BUF_CTL2_USBC3
        value = DisplayArgs.read_register(offset, gfx_index)
        port_buf_ctl = Gen15DdiRegs.REG_PORT_BUF_CTL2(offset, value)
        self.PowerwellDdiHIo = get_ddi_io(port_buf_ctl)

        offset = Gen15DdiRegs.OFFSET_PORT_BUF_CTL2.PORT_BUF_CTL2_USBC4
        value = DisplayArgs.read_register(offset, gfx_index)
        port_buf_ctl = Gen15DdiRegs.REG_PORT_BUF_CTL2(offset, value)
...
enabled_power_well_mask.value = ENABLED_POWER_WELL_MASK_LNL
