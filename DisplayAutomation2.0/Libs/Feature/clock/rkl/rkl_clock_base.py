##
# @file   rkl_clock_base.py
# @brief  Base class for doing RKL CD clock and port clock validation
...
from Libs.Feature.clock.rkl import rkl_clock_edp
from Libs.Feature.clock.rkl import rkl_clock_hdmi
from registers.mmioregister import MMIORegister


##
# @brief RKL clock verification class for CDCLK and port CLK
class RklClock:
    ...
    cdclock_map = dict([
        (19.2, dict([(172.8, 36), (192, 40), (307.2, 64), (326.4, 136), (556.8, 116), (652.8, 136)])),
        (38.4, dict([(172.8, 18), (192, 20), (307.2, 32), (326.4, 68), (556.8, 58), (652.8, 68)])),
        (24, dict([(180, 30), (192, 32), (312, 52), (324, 108), (552, 92), (648, 108)]))
    ])
    # Reference frequency to CD clock map for CD CLK divider
    cd2x_divider_map = dict([
        (19.2, dict([(172.8, 2), (192, 2), (307.2, 2), (326.4, 4), (556.8, 2), (652.8, 2)])),
        (38.4, dict([(172.8, 2), (192, 2), (307.2, 2), (326.4, 4), (556.8, 2), (652.8, 2)])),
        (24, dict([(180, 2), (192, 2), (312, 2), (324, 4), (552, 2), (648, 2)]))
    ])
    ...
    ##
    # @brief Generic function for verifying RKL clock
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        if str(display_port).upper().__contains__('DP_'):
            clock = rkl_clock_edp.RklClockEdp()
        else:
            clock = rkl_clock_hdmi.RklClockHdmi()
    ...
    def get_optimal_cd_clock_from_pixelclock(self, gfx_index: str, max_pixel_rate: float, display_list: list) -> float:
        ...
        if is_efp_present is True:
            ...
            if reference_freq == 24:
                optimal_cdclock = 180 if supported_pixel_rate < 360 else \
                    192 if supported_pixel_rate < 384 else \
                        312 if supported_pixel_rate < 624 else \
                            324 if supported_pixel_rate < 648 else \
                                552 if supported_pixel_rate < 1104 else 648
            # Cd Clock values for 19.2 and 38.4 Mhz Reference Frequency
            else:
                optimal_cdclock = 172.8 if supported_pixel_rate < 345.6 else \
                    192 if supported_pixel_rate < 384 else \
                        307.2 if supported_pixel_rate < 614.4 else \
                            326.4 if supported_pixel_rate < 652.8 else \
                                556.8 if supported_pixel_rate < 1113.6 else 652.8
