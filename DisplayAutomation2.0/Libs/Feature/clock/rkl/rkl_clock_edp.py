##
# @file    rkl_clock_edp.py
# @brief   This tests EDP PLLS are programmed correctly for platform RKL. Can be used in EDP tests.
# @author  Kruti Vadhavaniya

import logging

from Libs.Feature.clock import clock_helper as clk_helper
from Libs.Feature.clock.rkl import rkl_clock_registers
from Libs.Feature.display_port import dpcd_helper
from Libs.Core.logger import gdhm

clock_helper = clk_helper.ClockHelper()
rkl_clock_reg = rkl_clock_registers.RklClockRegisters()
edp = 0
...
##
# @brief RKL edp clock verification class
class RklClockEdp():
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        if str(display_port).upper().__contains__('_A'):
            ...
            dpllValue = clock_helper.get_value_by_range(reg_value, 27, 28, self.ddi_pll_map, 'DPLL')
        ...
        if (ret is True):
            if (True == self.verify_edp_pll(pll_values, display_port, gfx_index)):
                ...
                gdhm.report_bug(
                    title="[Interfaces][Display_Engine]RKL_Clock_EDP: PLL Register values {0} not programmed as "
            ...
            gdhm.report_bug(
                title="[Interfaces][Display_Engine]RKL_Clock_EDP:Failed getting reference pll values"
    ...
    def get_edp_pll_ref_values(self, link_bw, ref_clk):
        ...
        elif ((ref_clk == 19.2) or (ref_clk == 38.4)):
            if link_bw == 5.4:
