...
##
# @brief RKL port clock verification class for HDMI display
class RklClockHdmi():
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        dpllValue = clock_helper.get_value_by_range(reg_value, 0, 1, self.ddi_pll_map, 'DPLL') if str(
            display_port).upper().__contains__('HDMI_A') else clock_helper.get_value_by_range(reg_value, 2, 3,
                                                                                              self.ddi_pll_map,
                                                                                              'DPLL') if str(
            display_port).upper().__contains__('HDMI_B') else clock_helper.get_value_by_range(reg_value, 4, 5,
                                                                                              self.ddi_pll_map,
                                                                                              'DPLL') if str(
            display_port).upper().__contains__('HDMI_C') else clock_helper.get_value_by_range(reg_value, 27, 28,
                                                                                              self.ddi_pll_map, 'DPLL')

        status = rkl_clock_reg.verify_pll_enable(display_port, dpllValue, gfx_index)
    ...
    def calculate_symbol_freq(self, display_port, gfx_index='gfx_0'):
        pipe = rkl_clock_reg.get_pipe_for_port(display_port, gfx_index)
    ...
    def get_div_values(self, bestdiv):
        if bestdiv != 0:
            ...
            gdhm.report_bug(
                title="[Interfaces][Display_Engine]RKL_Clock_HDMI: Best divider value not found while calculating the "
                      "best DCO Frequency",
