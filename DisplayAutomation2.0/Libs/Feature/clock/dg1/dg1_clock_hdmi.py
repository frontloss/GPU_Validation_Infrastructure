...
##
# @brief DG1 port clock verification class for HDMI display
class Dg1ClockHdmi():
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        dpllValue = clock_helper.get_value_by_range(reg_value_DPCLKA0_CFGCR0, 0, 1, self.ddi_pll_0_1_map,
                                                    'DPLL') if str(
            display_port).upper().__contains__('HDMI_A') else clock_helper.get_value_by_range(reg_value_DPCLKA0_CFGCR0,
                                                                                              2, 3,
                                                                                              self.ddi_pll_0_1_map,
                                                                                              'DPLL') if str(
            display_port).upper().__contains__('HDMI_B') else clock_helper.get_value_by_range(reg_value_DPCLKA1_CFGCR0,
                                                                                              0, 1,
                                                                                              self.ddi_pll_2_3_map,
                                                                                              'DPLL') if str(
            display_port).upper().__contains__('HDMI_C') else clock_helper.get_value_by_range(reg_value_DPCLKA1_CFGCR0,
                                                                                              2, 3,
                                                                                              self.ddi_pll_2_3_map,
                                                                                              "DPLL")

        status = dg1_clock_reg.verify_pll_enable(display_port, dpllValue, gfx_index)
    ...
    def calculate_symbol_freq(self, display_port, gfx_index='gfx_0'):
        pipe = dg1_clock_reg.get_pipe_for_port(display_port, gfx_index)
    ...
    def get_div_values(self, bestdiv):
        if (bestdiv != 0):
            ...
            gdhm.report_bug(
                title="[Interfaces][Display_Engine]DG1_Clock_HDMI: Best divider value not found while calculating the "
                      "best DCO Frequency",
