...
##
# @brief TGL Port clock verification class for MIPI display
class TglClockMipi():
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        status = tgl_clock_reg.verify_pll_enable(display_port, dpllValue, gfx_index)
    ...
    def ValidateDPll(self, display_port, gfx_index='gfx_0'):
        ...
        self.esc_clock_divider = math.ceil(self.symbol_freq * (self.bpp / self.lane_count) / 19)
        ...
        # DPHY_ESC_CLK_DIV_REGISTER is not valid for LFK1. It has to be programmed to same as DSI_ESC_CLK_DIV_REGISTER
        # regValue = clock_helper.clock_register_read('DPHY_ESC_CLK_DIV_REGISTER', 'DPHY_ESC_CLK_DIV_0')
        ...
        # TGL WA to divide dco fraction part by 2, this will override bspec formula
        ...
        ret &= clock_helper.verify_port_clock_programming_ex(feature="DSI_ESC_CLK_DIV_DSI0 & DSI1",
                                                             ...
                                                             expected=[int(self.esc_clock_divider)], actual=[dsi_esc_div])

        # ret &= clock_helper.verify_clock_programming(dphy_esc_div, self.esc_clock_divider,
        #                                             'DPHY_ESC_CLK_DIV: escape_clock_divider_m')
        ...
        if abs(calculated_pll_freq - driver_pll_frequency) > reference_frequency / (
            ...
            logging.warning("WARN: {0}: Current Value = {1} MHz Greater than 1 MHz of Expected Value {2} MHz"
                            .format('Driver PLL Frequency', str(driver_pll_frequency), str(calculated_pll_freq)))
            return ret
        else:
            logging.debug("PASS: {0}: Current Value = {1} MHz Less than 1 MHz of Expected Value {2} MHz"
                          .format('Driver PLL Frequency', str(driver_pll_frequency), str(calculated_pll_freq)))
            return ret
    ...
    def get_div_values(self, bestdiv):
        if bestdiv != 0:
            ...
            gdhm.report_bug(
                title="[Interfaces][Display_Engine]TGL_Clock_MIPI: Best divider value not found while calculating the "
                      "best DCO Frequency",
