...
##
# @brief TGL port clock verification class for HDMI display
class TglClockHdmi():
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        dpllValue = clock_helper.get_value_by_range(reg_value, 0, 1, self.ddi_pll_map, 'DPLL') if str(
            display_port).upper().__contains__('HDMI_A') else clock_helper.get_value_by_range(reg_value,
                                                                                              2, 3,
                                                                                              self.ddi_pll_map,
                                                                                              'DPLL') if str(
    ...
    # @param[in] display_port - Display port
    # @param[in] gfx_index - Graphics index on which clock verificatio
    ...
    def get_div_values(self, bestdiv):
        if (bestdiv != 0):
            ...
            gdhm.report_bug(
                title="[Interfaces][Display_Engine]TGL_Clock_HDMI: Best divider value not found while calculating the "
                      "best DCO Frequency",
