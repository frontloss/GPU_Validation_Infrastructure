##
# @file   tgl_clock_base.py
# @brief  Base class for doing TGL CD clock and port clock validation
# @author Kruti Vadhavaniya

import logging

from Libs.Core import display_utility, registry_access
from Libs.Feature.clock import clock_helper as clk_helper
from Libs.Feature.clock.tgl import tgl_clock_dp_dekelphy
from Libs.Feature.clock.tgl import tgl_clock_edp
from Libs.Feature.clock.tgl import tgl_clock_hdmi
from Libs.Feature.clock.tgl import tgl_clock_hdmi_dekelphy
from Libs.Feature.clock.tgl import tgl_clock_mipi
from registers.mmioregister import MMIORegister


##
# @brief TGL clock verification class for CDCLK and port CLK
class TglClock:
    # CD Clock map as per Bspec Ref: https://gfxspecs.intel.com/Predator/Home/Index/7385
    ...
    cdclock_map = dict([
        (19.2, dict([(18, 172.8), (20, 192), (32, 307.2), (58, 556.8), (68, [326.4, 652.8])])),
        (38.4, dict([(9, 172.8), (10, 192), (16, 307.2), (29, 556.8), (34, [326.4, 652.8])])),
        (24, dict([(15, 180), (16, 192), (26, 312), (46, 552), (54, [324, 648])]))
    ])
    ...
    ##
    # @brief Generic function for verifying TGL clock
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        target_id = clock_helper_.get_target_id(display_port, gfx_index='gfx_0')
        ...
        # for HDMI_A or HDMI_B or HDMI_C, use combo phy
        if str(display_port).upper().__contains__('HDMI_A') or str(display_port).upper().__contains__('HDMI_B') \
                or str(display_port).upper().__contains__('HDMI_C'):
            clock = tgl_clock_hdmi.TglClockHdmi()
        # for DP_A or DP_B or DP_C (either edp or DP), use combo phy
        elif str(display_port).upper().__contains__('DP_A') or str(display_port).upper().__contains__(
                'DP_B') or str(display_port).upper().__contains__('DP_C'):
            clock = tgl_clock_edp.TglClockEdp()
        elif str(display_port).upper().__contains__('MIPI'):
            clock = tgl_clock_mipi.TglClockMipi()
        elif str(display_port).upper().__contains__('DP_'):
            clock = tgl_clock_dp_dekelphy.TglClockDpDekelPhy()
        else:
            clock = tgl_clock_hdmi_dekelphy.TglClockHdmiDekelPhy()
    ...
    def verify_sanity(self, display_port, gfx_index='gfx_0'):
        ...
        cd2x_divider = clock_helper_ \
            .get_value_by_range(reg_value, 22, 23, '', "CDCLK_CTL: CD2X_Divider_Select")
        ...
        pll_ratio = clock_helper_ \
            .get_value_by_range(reg_value, 0, 7, '', "PLL Ratio")
        ...
        reg_value = clock_helper_.clock_register_read('DPCLKA_CFGCR0_REGISTER', 'DPCLKA_CFGCR0', gfx_index)
        ddi_clock_map_enabled = {k: v for k, v in self.ddi_clock_map.items() if v & reg_value == 0}
    ...
    def get_current_cd_clock(self, gfx_index='gfx_0') -> int:
        clock_helper_ = clk_helper.ClockHelper()
        reg_value = clock_helper_.clock_register_read('CDCLK_CTL_REGISTER', 'CDCLK_CTL', gfx_index)
    ...
    def verify_cdclock(self, display_list, gfx_index='gfx_0'):
        verify = True
        is_seamless_divider_change = False
        clock_helper_ = clk_helper.ClockHelper()
        reg_value = clock_helper_.clock_register_read('CDCLK_CTL_REGISTER', 'CDCLK_CTL', gfx_index)
        cdclock_ctl_freq = clock_helper_ \
            .get_value_by_range(reg_value, 0, 10, self.cdclock_ctl_freq_dict, "CDCLK_CTL: CD Frequency Decimal")

        '''if dynamic cd clk is not enabled in VBT and SeamlessDividerChangeOnly registry added, CD CLK value can change 
        between Max and Max/2 CD CLK value - RCR - VSRI-5434'''
        if not clock_helper_.is_dynamic_cdclock_enabled(gfx_index):
            ...
            registry_value, registry_type = registry_access.read(args=ss_reg_args, reg_name="SeamlessDividerChangeOnly")
            if registry_value is None:
                logging.info("SeamlessDividerChangeOnly Registry not added")
                return True
            elif registry_value == 1:
                logging.info("SeamlessDividerChangeOnly registry value {}".format(registry_value))
                logging.info("Dynamic CD CLK disable and seamless Divider change enable")
                is_seamless_divider_change = True
            else:
                logging.info("SeamlessDividerChangeOnly registry value {} ".format(registry_value))
                return True
        # get bios-programmed max cd clock supported by the system
        system_max_cd_clk = self.get_system_max_cd_clk(gfx_index)
        if is_seamless_divider_change:
            ...
            # For DSC Displays with Slice Count of 1, Optimal Cd Clock using 2ppc method will not be sufficient to drive
            # the Display as only one DSC Engine will be active and this DSC Engine Clock will be synchronized with the
            # CD Clock not CD Clock x 2. Hence For DSC Displays with Slice Count 1 optimal cd clock will be determined
            # with 1ppc method. For this purpose, pixel clock of dsc display will be multiplied by 2 and the resultant
            # max pixel rate will be used to determine optimal cd clock
            ...
            if is_efp_present is True:
                if system_max_cd_clk in [648, 652.8]:
                    expected_cdclock = system_max_cd_clk / 2 if supported_pixel_rate < system_max_cd_clk \
                ...
                if reference_freq == 24:
                    expected_cdclock = 324 if supported_pixel_rate < 648 else 648
                # Cd Clock values for 19.2 amd 38.4 Mhz Reference Frequency
                else:
                    expected_cdclock = 326.4 if supported_pixel_rate < 652.8 else 652.8
        else:
    ...
    def get_optimal_cd_clock_from_pixelclock(self, gfx_index: str, max_pixel_rate: float, display_list: list) -> float:
        ...
        if is_efp_present is True:
            ...
            # Cd Clock values for 24 Mhz reference Frequency
            if reference_freq == 24:
                optimal_cdclock = 180 if supported_pixel_rate < 358 else \
                    192 if supported_pixel_rate < 382 else \
                        312 if supported_pixel_rate < 622 else \
                            324 if supported_pixel_rate < 646 else \
