##
# @file     tgl_clock_edp.py
# @brief  This tests EDP PLLS are programmed correctly for platform TGL. Can be used in EDP tests.
# @author  P, Raghupathy ; Ap, Kamal; Rajiv, Radhakrishnan; Doriwala, Nainesh P
...
##
# @brief Central frequency mapping dictiona
...
##
# @brief TGL Port clock verification class for edp/Dp display
class TglClockEdp():
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        if str(display_port).upper().__contains__('_A'):
            dpllValue = clock_helper.get_value_by_range(reg_value, 0, 1, self.ddi_pll_map, 'DPLL')
        elif str(display_port).upper().__contains__('_B'):
            dpllValue = clock_helper.get_value_by_range(reg_value, 2, 3, self.ddi_pll_map, 'DPLL')
        else:
            dpllValue = clock_helper.get_value_by_range(reg_value, 4, 5, self.ddi_pll_map, 'DPLL')
        ...
        if (ret is True):
            if (True == self.verify_edp_pll(pll_values, display_port, gfx_index)):
                ...
                gdhm.report_bug(
                    title="[Interfaces][Display_Engine]TGL_Clock_EDP: PLL Register values {0} not programmed as "
            ...
            gdhm.report_bug(
                title="[Interfaces][Display_Engine]TGL_Clock_EDP:Failed getting reference pll values"
