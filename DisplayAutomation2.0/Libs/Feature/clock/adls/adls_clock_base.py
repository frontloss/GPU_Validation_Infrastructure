##
# @file    adls_clock_base.py
# @brief   Base class for doing ADL-S clock validation
...
from Libs.Feature.clock.adls import adls_clock_edp
from Libs.Feature.clock.adls import adls_clock_hdmi
from registers.mmioregister import MMIORegister


##
# @brief ADLS clock verification class for CDCLK and port CLK
class AdlsClock:
    ...
    # TODO: Need to check which register exactly gives this value for ADLS
    ...
    # Reference Frequency to Cd Clock map as per bspec Ref: https://gfxspecs.intel.com/Predator/Home/Index/53720
    cdclock_map = dict([
    ...
    # Map of reference freq dictionary
    dssm_ref_freq_map = dict([
        (24, 0),
        (19.2, 1),
        (38.4, 2)
    ])
    # Map of Pipe allocation
    ...
    ##
    # @brief Generic function for verifying ADLS clock
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        if str(display_port).upper().__contains__('DP_'):
            clock = adls_clock_edp.AdlsClockEdp()
        else:
            clock = adls_clock_hdmi.AdlsClockHdmi()
    ...
    def verify_sanity(self, display_port, gfx_index='gfx_0'):
        ...
        cd2x_divider_str = 'Div by 1' if cd2x_divider == 0 else 'Div by 1.5' \
            if cd2x_divider == 1 else 'Div by 2' \
            if cd2x_divider == 2 else 'Div by 4'
    ...
    ##
    # @brief Generic function for verifying ADLS CD clock
