##
# @file   adls_clock_registers.py
# @brief  Python helper class for doing ADLS generic functions
...
##
# @brief ADLS clock Registers class
class AdlsClockRegisters:
    ...
    trans_ddi_port_map = dict([
        ('A', 1),
        ('B', 4),  # Port B drive through DDI-TC1
        ('C', 5),  # Port C drive through DDI-TC2
        ('D', 6),  # Port D drive through DDI-TC3
        ('E', 7)  # Port E drive through DDI-TC4
    ])
    ...
    def get_pipe(self, gfx_index='gfx_0'):
        ...
        return dict(map(lambda kv:
                        (
                            kv[0],
                            clock_helper.get_value_by_range(kv[1], 27, 30, self.trans_ddi_port_map, str(kv[0])))
                        , trans_map.items()))
