...
##
# @brief ADLP port clock verification class for HDMI display
class AdlpClockHdmi():
    ...
    # Map of possible ddi and clock map
    ddi_clock_map = dict([
        ('A', 1024),
        ('B', 2048),
        ('C', 4096),
    ...
    def verify_clock(self, display_port, gfx_index='gfx_0'):
        ...
        if str(display_port).upper().__contains__('HDMI_A'):
            dpllValue = clock_helper.get_value_by_range(reg_value, 0, 1, self.ddi_pll_map, 'DPLL')
        elif str(display_port).upper().__contains__('HDMI_B'):
        ...
        status = adlp_clock_reg.verify_pll_enable(display_port, dpllValue, gfx_index)
    ...
    def ValidateDPll(self, display_port, gfx_index='gfx_0'):
        ...
        logging.debug("INFO : Reference Frequency is {0}".format(str(reference_frequency)))
        ...
        ret &= clock_helper.verify_port_clock_programming_ex(feature='{}'.format(ClockRegister.DPLL_SSC),
                                                             ...
                                                             expected=[0],
                                                             actual=[reg_read_hdmi.SSC_ENABLE])
        ...
        if abs(calculated_pll_freq - driver_pll_frequency) > reference_frequency / (
            ...
            logging.debug("PASS: {0}: Current Value = {1} MHz is Less than 1 MHz of Expected Value {2} MHz".format(
    ...
    def calculate_symbol_freq(self, display_port, gfx_index='gfx_0'):
        ...
        pipe, ddi = disp_base.GetPipeDDIAttachedToPort(display_port, gfx_index='gfx_0')
    ...
    def calculate_best_dco_freq(self):
        ...
        dividerlist_new = list(
            filter(lambda x: ((x * 5 * self.symbol_freq) < dco_max) and ((x * 5 * self.symbol_freq) > dco_min),
                   self.dividerlist))
    ...
    def get_div_values(self, bestdiv):
        if bestdiv != 0:
            ...
            gdhm.report_bug(
                title="[Interfaces][Display_Engine]ADLP_Clock_HDMI: Best divider value not found while calculating the "
                      "best DCO Frequency",
