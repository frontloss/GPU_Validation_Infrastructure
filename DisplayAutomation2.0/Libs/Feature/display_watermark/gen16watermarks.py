######################################################################################
# @file             gen16watermarks.py
# @addtogroup       PyLibs_DisplayWatermark
# @brief            GEN16 specific module for DisplayWatermark
...
from Libs.Feature.display_watermark import gen16_dbuf_table


##
# @brief        This class contains Gen16 watermark and dbuf verification API's
class Gen16Watermarks(watermark_base.DisplayWatermarkBase):
    max_pipes = wm_util.MAX_PIPES_GEN16
    max_planes = wm_util.MAX_PLANES_GEN16
    sagv_latency = 0

    ##
    # @brief        Gen16Watermarks default __init__ function
    ...
    ##
    # @brief        Verifies watermark values programmed by driver for GEN16
    ...
    def verify_watermarks(self, is_48hz_verification=False, gfx_index='gfx_0', min_dbuf_check=False):
        ...
        latency = self.gen16_get_memory_latency()
    ...
    # @return       GEN16WATERMARK() object containing list of all active plane expected watermark blocks and lines
    def get_expected_watermarks(self, plane_list, pipe_list, latency, workarounds, pipe_count, plane_count):
        ...
        for pipe_idx in range(0, pipe_count):
            ...
            adjusted_pipe_pixel_rate = (adjusted_pipe_pixel_rate * 2) if pipe_list[pipe_idx].bIsInterlaced is True else adjusted_pipe_pixel_rate
            ...
            for plane_idx in range(0, plane_count):
                ...
                for level in range(0, wm_util.LATENCY_LEVELS):
                    ...
                    adjusted_plane_pixel_rate = round(adjusted_pipe_pixel_rate * plane_list[pipe_idx * plane_count + plane_idx].fPlaneDownscalingFactor, 2)
                    bpp = plane_list[pipe_idx * plane_count + plane_idx].uiPlaneBpp
                    pixel_format_text = self.get_pixel_format_text(plane_list[pipe_idx * plane_count + plane_idx].uiPixelFormat)
                    ...
                    if adjusted_plane_pixel_rate > 0 and plane_status is True:
                        ...
                        if tiling == wm_util.SURFACE_MEMORY_LINEAR:
                            ...
                            # ToDo: Need to update once closure from Arch
                            ...
                            blocks_per_line = math.ceil((tile4_min_lines * bytes_per_line / dbuf_block_size) + 1) / tile4_min_lines
                        else:
                            blocks_per_line = math.ceil(bytes_per_line / dbuf_block_size) + 1
                        method2 = round(math.ceil(current_latency * adjusted_plane_pixel_rate / pipe_hor_total) * blocks_per_line, 2)
                        method2_sagv = round(math.ceil(self.sagv_latency * adjusted_plane_pixel_rate / pipe_hor_total) * blocks_per_line, 2)
                        ...
                        if level == 0:
                            ...
                            if self.platform in ['ptl']:
                                ...
                                logging.info(f"exp_plane_wm_sagv_obj.bStatus {exp_plane_wm_sagv_obj.bStatus}")
                                ...
                                logging.debug(
                                    ...
                                    "M2={} YMin={} ResBlk={} ResLin={} MinDbuf={}".format(
                            ...
                            # Compare against max and update resultblock
    ...
    def gen16_get_memory_latency(self):
        latency = [0 for _ in range(6)]

        # If latency WM0 is zero then add fabric latency of 6 micros seconds
        fabric_latency = 6
    ...
    def validate_dbuf_distribution(self, pipe_list, plane_list, pipe_count, plane_count, gfx_index='gfx_0'):
        ...
        gen16_dbuf_distribution = gen16_dbuf_table.gen16_ptl_dbuf_distribution
        ...
        current_dbuf_allocations = gen16_dbuf_distribution[enabled_pipes]['allocation']
        current_enabled_slices = gen16_dbuf_distribution[enabled_pipes]['slices_enabled']
        ...
        current_mbus_joined = gen16_dbuf_distribution[enabled_pipes]['mbus_joined']
