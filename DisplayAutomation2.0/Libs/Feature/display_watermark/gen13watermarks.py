######################################################################################
# @file             gen13watermarks.py
# @addtogroup       PyLibs_DisplayWatermark
# @brief            GEN13 specific module for DisplayWatermark
...
from Libs.Feature.display_watermark import gen13_dbuf_table
from Libs.Core.logger import gdhm


##
# @brief        This class contains Gen13 watermark and dbuf verification API's
class Gen13Watermarks(watermark_base.DisplayWatermarkBase):
    max_pipes = wm_util.MAX_PIPES_GEN13
    max_planes = wm_util.MAX_PLANES_GEN13
    sagv_latency = 0

    ##
    # @brief        Gen13Watermarks default __init__ function
    ...
    ##
    # @brief        Verifies watermark values programmed by driver for GEN13
    ...
    def verify_watermarks(self, is_48hz_verification=False, gfx_index = 'gfx_0', min_dbuf_check=False):
        ...
        latency = self.gen13_get_memory_latency(pipe_list, max_pipes, gfx_index)
        ...
        if status is True:
            ...
            if self.platform == 'adlp':
                status_sagv = self.verify_trans_sagv_watermark(expected_cur_trans_sagv_wm, max_pipes, max_planes,
                                                               plane_list, is_cursor_wm=True, gfx_index=gfx_index)
                status &= status_sagv
        else:
    ...
    def read_latency_value_from_mailbox(self, gfx_index='gfx_0'):
        ...
        if self.platform == 'dg2':
            ...
            self.sagv_latency = (data_0 & 0x000000FF)
        logging.debug("SAGV latency set = %s" % hex(self.sagv_latency))
    ...
    # @param[in]    gfx_index as optional
    # @return       list containing latencies
    def gen13_get_memory_latency(self, pipe_list, pipe_count, gfx_index='gfx_0'):
        ...
        # ADLP has only 6 latency values as compared to 8 in other platforms
        if self.platform == 'adlp':
            latency = latency[:-2]
            ...
            logging.debug('Value of DisplayFeatureControl registry : {0}'.format(hex(dfc_default_value)))

            # Set bit 20 to enable WM 2X Latencies
            enable_wm_2x_latencies = dfc_default_value | 0x100000
            if enable_wm_2x_latencies == dfc_default_value:
                logging.info('WM 2X Latencies is enabled in display feature control')
                for i in range(0, len(latency)):
                    latency[i] = latency[i] - fabric_latency

                latency = [element * 2 for element in latency]
    ...
    # @return       GEN13WATERMARK() object containing list of all active plane expected watermark blocks and lines
                ...
                for level in range(0, wm_util.LATENCY_LEVELS):
                    ...
                    if adjusted_plane_pixel_rate > 0 and plane_status is True:
                        ...
                        if tiling == wm_util.SURFACE_MEMORY_LINEAR:
                            ...
                            # DG2 supports Tile4 and ADLP supports Y Tile. Both take the same WM calculation path
                        ...
                        if tiling in [wm_util.SURFACE_MEMORY_TILED_4, wm_util.SURFACE_MEMORY_Y_LEGACY_TILED]:
                            # DG2 supports Tile4 and ADLP supports Y Tile. Both take the same WM calculation path
                        ...
                        if level == 0:
                            ...
                            logging.debug("TRANS_WM_{}_{} tile={}({}) TransResBlk={} SelResBlks={} TransYmin={}"
                                          .format(wm_util.PLANE_NAME[plane_idx], wm_util.PIPE_NAME[pipe_idx],
                                                  tiling, tiling_text, sel_result_blocks, transition_y_min,
                                                  exp_trans_wm_obj.fResultBlocks,))
    ...
    def validate_dbuf_distribution(self, pipe_list, plane_list, pipe_count, plane_count, gfx_index='gfx_0'):
        ...
        gen13_dbuf_distribution = gen13_dbuf_table.gen13_dg2_dbuf_distribution if self.platform == 'dg2' \
            else gen13_dbuf_table.gen13_adlp_dbuf_distribution
        ...
        if self.platform == 'adlp':
            # 6 credits should be programmed in single pipe A or single pipe B scenario.
            if enabled_pipes == 1:
                value = self.driver_interface_.mmio_read(wm_util.PIPE_MBUS_DBOX_CTL_A, gfx_index)
            elif enabled_pipes == 2:
                value = self.driver_interface_.mmio_read(wm_util.PIPE_MBUS_DBOX_CTL_B, gfx_index)
            elif enabled_pipes == 4:
                value = self.driver_interface_.mmio_read(wm_util.PIPE_MBUS_DBOX_CTL_C, gfx_index)
            elif enabled_pipes == 8:
                value = self.driver_interface_.mmio_read(wm_util.PIPE_MBUS_DBOX_CTL_D, gfx_index)
            # Multi pipe scenario - 4 credits expected for multi pipe scenarios
            elif enabled_pipes:
                PIPE_MBUS_REGISTERS = [wm_util.PIPE_MBUS_DBOX_CTL_A, wm_util.PIPE_MBUS_DBOX_CTL_B,
                                       wm_util.PIPE_MBUS_DBOX_CTL_C, wm_util.PIPE_MBUS_DBOX_CTL_D]
                for pipe_idx in range(pipe_count):
                    if pipe_list[pipe_idx].bIsPipeEnable:
                        value = self.driver_interface_.mmio_read(PIPE_MBUS_REGISTERS[pipe_idx], gfx_index)
                        programmed_credits = value & 0x0000000F
                        if programmed_credits == 4:
                            logging.info(f"\t PIPE_{chr(65+pipe_idx)} : Credits programmed in Multi pipe/Mbus join config expected = 4 Actual = 4")
                        else:
                            logging.error(f"\t PIPE_{chr(65+pipe_idx)} : Credits programmed in Multi pipe/Mbus join config expected = 4 "
                                          f"Actual = {programmed_credits}")
                            status = False

            #verify credits in case of single pipe scenarios
            if enabled_pipes in [1,2,4,8]:
                programmed_credits = value & 0x0000000F

                if enabled_pipes == 1 or enabled_pipes == 2:
                    if programmed_credits == 6:
                        logging.info(f"\t Credits programmed in Single pipe/Mbus join config expected = 6 Actual = 6")
                    else:
                        logging.error(f"\t Credits programmed in Single pipe/Mbus join config expected = 6 "
                                     f"Actual = {programmed_credits}")
                        status = False

                if enabled_pipes == 4 or enabled_pipes == 8:
                    if programmed_credits == 4:
                        logging.info(f"\t Credits programmed in Single pipe(C/D) config expected = 4 Actual = 4")
                    else:
                        logging.error(f"\t Credits programmed in Single pipe(C/D) config expected = 4 "
                                     f"Actual = {programmed_credits}")
                        status = False

        current_dbuf_allocations = gen13_dbuf_distribution[enabled_pipes]['allocation']
        current_enabled_slices = gen13_dbuf_distribution[enabled_pipes]['slices_enabled']
        ...
        ##
        # Verify MBus joined mode status (Not applicable for DG2)
        if self.platform != 'dg2':

            current_mbus_joined = gen13_dbuf_distribution[enabled_pipes]['mbus_joined']
            ...
            if current_mbus_joined:
                if mbus_joined_mode != getattr(mbus_ctl_value, 'mbus_joining_ENABLE'):
                    ...
                    gdhm.report_bug(
                        title="[OS][DBUF] MBUS Joining disabled",
                ...
                if mbus_joined_mode != getattr(mbus_ctl_value, 'mbus_joining_DISABLE'):
                    ...
                    gdhm.report_bug(
                        title="[OS][DBUF] MBUS joining enabled",
                        problem_classification=gdhm.ProblemClassification.FUNCTIONALITY,
                        component=gdhm.Component.Driver.DISPLAY_OS_FEATURES,
                        priority=gdhm.Priority.P2,
                        exposure=gdhm.Exposure.E1)
                ...
                for plane_idx in range(0, plane_count):
                    ...
                    if plane_obj.bStatus:
                        ...
                        if (((plane_buf_start != 0) or (plane_buf_end != 0)) and
                            ...
                            gdhm.report_bug(
                                title="[OS][DBUF] PLANE_{}_{} [DBuf:{}-{}] exceeds Pipe_{} [DBuf:{}-{}] DBuf allocation "
                                          "boundaries".format(wm_util.PLANE_NAME[plane_idx],
                                                               wm_util.PIPE_NAME[pipe_idx],
                                                               plane_buf_start, plane_buf_end,
                                                               wm_util.PIPE_NAME[pipe_idx],
            ...
            for plane_idx in range(plane_count):
                ...
                if plane_obj.bStatus:
                    ...
                    if plane_obj.uiBufferAllocated < min_dbuf_needed:
                        ...
                        # For Gen13+ platforms, for async flips, the actual dbuf allocated should be eactly equal to
