######################################################################################
# @file             gen17watermarks.py
# @addtogroup       PyLibs_DisplayWatermark
# @brief            GEN17 specific module for DisplayWatermark
...
from Libs.Feature.display_watermark import gen17_dbuf_table
from Libs.Core.logger import gdhm
from Libs.Feature.display_watermark.watermark import GDHM_FEATURE_TAG


##
# @brief        This class contains Gen17 watermark and dbuf verification API's
class Gen17Watermarks(watermark_base.DisplayWatermarkBase):
    max_pipes = wm_util.MAX_PIPES_GEN17
    max_planes = wm_util.MAX_PLANES_GEN17
    sagv_latency = 0

    ##
    # @brief        Gen17Watermarks default __init__ function
    ...
    ##
    # @brief        Verifies watermark values programmed by driver for GEN17
    ...
    def verify_watermarks(self, is_48hz_verification=False, gfx_index='gfx_0', min_dbuf_check=False):
        ...
        latency = self.gen17_get_memory_latency()
        ...
        if status is True:
            status = self.verify_lp_watermarks(expected_lp_wm, programmed_lp_wm, max_pipes, max_planes, plane_list,
                                               gfx_index)
            status_sagv = self.verify_sagv_watermark(expected_sagv_wm, max_pipes, max_planes, plane_list,
                                                     gfx_index=gfx_index)
            status &= status_sagv
    ...
    # @return       GEN17WATERMARK() object containing list of all active plane expected watermark blocks and lines
    def get_expected_watermarks(self, plane_list, pipe_list, latency, workarounds, pipe_count, plane_count):
        ...
        for pipe_idx in range(0, pipe_count):
            ...
            adjusted_pipe_pixel_rate = (adjusted_pipe_pixel_rate * 2) \
                if pipe_list[pipe_idx].bIsInterlaced is True else adjusted_pipe_pixel_rate
            ...
            for plane_idx in range(0, plane_count):
                ...
                for level in range(0, wm_util.LATENCY_LEVELS):
                    ...
                    if adjusted_plane_pixel_rate > 0 and plane_status is True:
                        ...
                        elif tiling == wm_util.SURFACE_MEMORY_TILED_4:
                            # If PanningEnabled = Plane_Offset Start X Pos !=0:
                            #   blocks_per_line = ceiling[(tile4_min_lines * bytes_per_line/ dbuf_block_size)
                            #   + PanningEnabled] / tile4_min_lines
                            blocks_per_line = math.ceil((tile4_min_lines *
                                                         bytes_per_line / dbuf_block_size) + 1) / tile4_min_lines
                        else:
                            blocks_per_line = math.ceil(bytes_per_line / dbuf_block_size) + 1
                        ...
                        # Step-3 Calculate Tile 4 minimum
                        tile4_min_blocks = tile4_min_lines * blocks_per_line

                        # Step-4 Select Watermark result
                        if tiling != wm_util.SURFACE_MEMORY_TILED_4:
                            sel_result_blocks = method2
                        else:
                            sel_result_blocks = max(method2, tile4_min_blocks)

                        if tiling != wm_util.SURFACE_MEMORY_TILED_4:
                            sel_result_blocks_sagv = method2_sagv
                        else:
                            sel_result_blocks_sagv = max(method2_sagv, tile4_min_blocks)

                        # Step-5 Convert Result to block and lines
                        result_blocks = math.ceil(sel_result_blocks)
                        result_blocks_sagv = math.ceil(sel_result_blocks_sagv)
                        ...
                        if tiling in [wm_util.SURFACE_MEMORY_TILED_4]:
                            if result_lines % tile4_min_lines == 0:
                                extra_lines = tile4_min_lines
                                extra_lines_sagv = tile4_min_lines
                            else:
                                extra_lines = (tile4_min_lines * 2) - (result_lines % tile4_min_lines)
                                extra_lines_sagv = (tile4_min_lines * 2) - (result_lines_sagv % tile4_min_lines)
                            min_dbuf = (result_lines + extra_lines) * blocks_per_line
                            min_dbuf_sagv = (result_lines_sagv + extra_lines_sagv) * blocks_per_line
                        else:
                            min_dbuf = math.ceil(result_blocks + (result_blocks * 1.1))
                            min_dbuf_sagv = math.ceil(result_blocks_sagv + (result_blocks_sagv * 1.1))
                        ...
                        if exp_plane_wm_obj.bStatus is True and level > 0:
                        ...
                        logging.debug(
                            ...
                            "M2={} YMin={} ResBlk={} ResLin={}".format(
                                wm_util.PLANE_NAME[plane_idx], wm_util.PIPE_NAME[pipe_idx], level,
                                adjusted_plane_pixel_rate, bpp,
                                source_width, tiling, tiling_text, pipe_hor_total, blocks_per_line, method2,
                                tile4_min_blocks, exp_plane_wm_obj.fResultBlocks, exp_plane_wm_obj.fResultLines))
                        ...
                        if level == 0:
                            ...
                            # Calculate transition tile 4 minimum
                            transition_4_min = 2 * tile4_min_blocks
                            ...
                            if (tiling == wm_util.SURFACE_MEMORY_X_TILED) or (
                                ...
                                trans_result_blocks = max(sel_result_blocks, transition_4_min) + \
                                                      transition_offset_blocks
                            trans_result_blocks = math.ceil(trans_result_blocks) + 1
                            # Compare against max and update result block
                            ...
                            if self.platform in ['nvl', 'cls']:
                                ...
                                logging.debug(
                                    "SAGV_WM_{}_{} adjustedpixclk={} bpp={} width={} tile={}({}) htotal={} "
                                    "blkperline={} M2={} YMin={} ResBlk={} ResLin={} MinDbuf={}".format(
                                        wm_util.PLANE_NAME[plane_idx], wm_util.PIPE_NAME[pipe_idx],
                                        adjusted_plane_pixel_rate, bpp, source_width, tiling, tiling_text,
                                        pipe_hor_total, blocks_per_line, method2_sagv,
                                        tile4_min_blocks, exp_plane_wm_sagv_obj.fResultBlocks,
                                ...
                                if (tiling == wm_util.SURFACE_MEMORY_X_TILED) or \
                                    ...
                                    trans_result_blocks_sagv = max(sel_result_blocks_sagv, transition_4_min) + \
                                                               transition_offset_blocks
                                ...
                                logging.debug("TRANS_SAGV_WM_{}_{} tile={}({}) TransResBlk={} SelResBlks={} "
                                              ...
                                              .format(wm_util.PLANE_NAME[plane_idx], wm_util.PIPE_NAME[pipe_idx],
                                                      tiling, tiling_text, sel_result_blocks_sagv, transition_4_min,
    ...
    def gen17_get_memory_latency(self):
        latency = [0 for _ in range(6)]
        # Add fabric latency of 6ms
        fabric_latency = 6
        ...
        if current_exec_env in ["SIMENV_FULSIM", "SIMENV_PIPE2D"]:
            ...
            logging.info("Pre-Si: Memory Latency values: SAGV {0} LATENCY {1}"
                         .format(self.sagv_latency, latency))
        else:
            ...
            logging.info("Punit values: LATENCY_SAGV {0} LATENCY_LP0_LP1 {1} LATENCY_LP2_LP3 {2} LATENCY_LP4_LP5 {3}".
    ...
    def validate_dbuf_distribution(self, pipe_list, plane_list, pipe_count, plane_count, gfx_index='gfx_0'):
        ...
        if self.platform in ['cls']:
            gen17_dbuf_distribution = gen17_dbuf_table.gen17_cls_dbuf_distribution
        else:
            gen17_dbuf_distribution = gen17_dbuf_table.gen17_nvl_dbuf_distribution
        ...
        current_dbuf_allocations = gen17_dbuf_distribution[enabled_pipes]['allocation']
        current_enabled_slices = gen17_dbuf_distribution[enabled_pipes]['slices_enabled']
        ...
        if not status:
            gdhm.report_test_bug_os(f"{GDHM_FEATURE_TAG} DBUF_CTL status mismatch - Expected PowerState Enable, "
                                    f"Actual Disabled")

        ##
        # Verify MBus joined mode status
        current_mbus_joined = gen17_dbuf_distribution[enabled_pipes]['mbus_joined']
        ...
        if current_mbus_joined:
            if mbus_joined_mode != getattr(mbus_ctl_value, 'mbus_joining_ENABLE'):
                ...
                gdhm.report_test_bug_os(f"{GDHM_FEATURE_TAG} MBUS_CTL status mismatch - Expected Joining Enable, "
                                        f"Actual Disabled")
                status = False
            else:
            ...
            if mbus_joined_mode != getattr(mbus_ctl_value, 'mbus_joining_DISABLE'):
                ...
                gdhm.report_test_bug_os(f"{GDHM_FEATURE_TAG} MBUS_CTL status mismatch - Expected Joining Disable, "
                                        f"Actual Enabled")
                status = False
            else:
        ...
        for pipe_idx in range(0, pipe_count):
            if pipe_list[pipe_idx].bIsPipeEnable is True:
                ...
                cursor_buf = self.mmio_read.read('PLANE_BUF_CFG_REGISTER', cursor_buf_reg, self.platform,
                                                 gfx_index=gfx_index)
    ...
    def check_min_dbuf_needed(self, plane_list, plane_count, pipe_count, is_async_flip_check=False):
        ##
        # BSpec link: https://gfxspecs.intel.com/Predator/Home/Index/68907
            ...
            for plane_idx in range(plane_count):
                ...
                if plane_obj.bStatus:
                    ...
                    if plane_obj.uiBufferAllocated < min_dbuf_needed:
                        # By default, actual dbuf allocated should be at least equal to or more than minimum dbuf
                        ...
                        logging.info("PASS: Minimum DBuf allocated for PLANE_{}_{}:"
                                     ...
                                     .format(wm_util.PLANE_NAME[plane_idx], wm_util.PIPE_NAME[pipe_idx],
                                             min_dbuf_needed, plane_obj.uiBufferAllocated))
        if status is False:
            gdhm.report_test_bug_os(f"{GDHM_FEATURE_TAG} Minimum DBuf not allocated for PLANE WM")
        return status
    ...
    def verify_sagv_watermark(self, exp_sagv_wm, pipe_count, plane_count, plane_list, is_cursor_wm=False,
                              gfx_index='gfx_0'):
            ...
            for plane_idx in range(0, plane_count):
                ...
                if plane_list[pipe_idx * plane_count + plane_idx].bStatus:
                    ...
                    if (exp_sagv_wm_status is True) and (prog_sagv_wm_status is True):
                        if (exp_sagv_wm_result_blocks != prog_sagv_wm_result_blocks) and (
                                exp_sagv_wm_result_lines != prog_sagv_wm_result_lines):
                            # WM Results line Mismatch
                            failure = '--> BLOCKS & LINES MISMATCH'
                            status = False
                        elif exp_sagv_wm_result_blocks != prog_sagv_wm_result_blocks:
                            # WM Results blocks Mismatch
                            failure = '--> BLOCKS MISMATCH'
                            status = False
                        elif exp_sagv_wm_result_lines != prog_sagv_wm_result_lines:
                            # WM Results line Mismatch
                            failure = '--> LINES MISMATCH'
                            status = False
        ...
        if wm_status is False:
            gdhm.report_test_bug_os(f"{GDHM_FEATURE_TAG} SAGV_WM Verification Failed - Blocks/Lines Mismatch ")
        return wm_status
    ...
    # @param[in]    gfx_index as optional
    # @return       bool
    def verify_trans_sagv_watermark(self, exp_trans_sagv_wm, pipe_count, plane_count, plane_list, is_cursor_wm=False,
                                    gfx_index='gfx_0'):
            ...
            if is_cursor_wm is False:
                ...
                for plane_idx in range(0, plane_count):
                    ...
                    plane_wm_offset = 'PLANE_WM_' + wm_util.PLANE_NAME[plane_idx] + '_' + \
                                      wm_util.PIPE_NAME[pipe_idx]
                    plane_wm_reg = self.mmio_read.read('PLANE_WM_REGISTER', plane_wm_offset, self.platform,
                                                       extra_offset=(level * wm_util.NEXT_REGISTER_OFFSET),
                                                       gfx_index=gfx_index)
                    ##
                    # Fetch the Programmed SAGV WM value
                    plane_wm.bStatus = True if (plane_wm_reg.__getattribute__("enable") !=
        ...
        for pipe_idx in range(0, pipe_count):
            for plane_idx in range(0, plane_count):
                ...
                if plane_list[pipe_idx * plane_count + plane_idx].bStatus:
                    ...
                    wm_status = wm_status & status
        if wm_status is False:
            gdhm.report_test_bug_os(f"{GDHM_FEATURE_TAG} TRANS_SAGV_WM Verification Failed - Blocks/Lines Mismatch ")
        return wm_status
