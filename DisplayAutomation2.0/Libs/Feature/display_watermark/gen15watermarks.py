######################################################################################
# @file             gen15watermarks.py
# @addtogroup       PyLibs_DisplayWatermark
# @brief            GEN15 specific module for DisplayWatermark
...
from Libs.Feature.display_watermark import gen15_dbuf_table


##
# @brief        This class contains Gen15 watermark and dbuf verification API's
class Gen15Watermarks(watermark_base.DisplayWatermarkBase):
    max_pipes = wm_util.MAX_PIPES_GEN15
    max_planes = wm_util.MAX_PLANES_GEN15
    sagv_latency = 0

    ##
    # @brief        Gen15Watermarks default __init__ function
    ...
    ##
    # @brief        Verifies watermark values programmed by driver for GEN15
    ...
    def verify_watermarks(self, is_48hz_verification=False, gfx_index='gfx_0', min_dbuf_check=False):
        ...
        latency = self.gen15_get_memory_latency()
    ...
    # @return       GEN15WATERMARK() object containing list of all active plane expected watermark blocks and lines
    ...
    def gen15_get_memory_latency(self):
    ...
    def validate_dbuf_distribution(self, pipe_list, plane_list, pipe_count, plane_count, gfx_index='gfx_0'):
        ...
        gen15_dbuf_distribution = gen15_dbuf_table.gen15_lnl_dbuf_distribution
        ...
        current_dbuf_allocations = gen15_dbuf_distribution[enabled_pipes]['allocation']
        current_enabled_slices = gen15_dbuf_distribution[enabled_pipes]['slices_enabled']
        ...
        current_mbus_joined = gen15_dbuf_distribution[enabled_pipes]['mbus_joined']
            ...
            for plane_idx in range(0, plane_count):
                ...
                if plane_list[pipe_idx * plane_count + plane_idx].bStatus:
                    ...
                    if (exp_sagv_wm_status is True) and (prog_sagv_wm_status is True):
                        if (exp_sagv_wm_result_blocks != prog_sagv_wm_result_blocks) and (exp_sagv_wm_result_lines != prog_sagv_wm_result_lines):
