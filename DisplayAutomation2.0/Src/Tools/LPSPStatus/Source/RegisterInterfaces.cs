...
namespace LPSPStatus
{
    public static class RegisterInterfaces
    {
        ...
        public static extern UInt32 readMMIOReg(UInt32 offset, ref UInt32 value);


        private const int D3DKMT_ESCAPE_DRIVERPRIVATE = 0;
        private const int TOOL_ESC_READ_MMIO_REGISTER = 0;
        private static bool escape_path;
        private static bool diva_driver_status;

        public static bool Escape_path
        {
            get
            {
                return escape_path;
        ...
        public static bool Diva_driver_status
        {
            get
            {
                return diva_driver_status;
            }

            set
            {
                diva_driver_status = value;
            }
        }

        public static void ReadMMIO(uint offset, out uint value)
        {
            if (escape_path == true)
                ReadWriteRegister(offset, out value);
            else
            {
                if (diva_driver_status == true)
                    DIVAMMIORead(offset, out value);
                else
                    RubyMMIORead(offset, out value);
            }
        }

        private static void DIVAMMIORead(uint offset, out uint value)
        {
            value = 0;
            ...
            MmioAccessArgs.Offset = offset;

            try
            {
                // Read MMIO
                DivaGfxAccessUtility.ReadMmio(MmioAccessArgs);
                value = MmioAccessArgs.Value;
            }
            catch (Exception Exp)
            {
                Logger.Message("ERROR: Exception in DIRECT-MMIO-READ: {0}", Exp.Message);
            }
        }

        private static void RubyMMIORead(uint offset, out uint value)
        {
            value = 0;
            readMMIOReg((uint)offset, ref value);
        }

        private static void ReadWriteRegister(uint offset, out uint value)
        {
