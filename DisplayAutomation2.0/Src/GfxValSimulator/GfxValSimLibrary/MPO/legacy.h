 ...
 */
CDLL_EXPORT UINT mainline_CheckForMultiPlaneOverlaySupport(PGFX_ADAPTER_INFO pAdapterInfo, PPLANES pPlanes);
 ...
 */
CDLL_EXPORT UINT mainline_SetSourceAddressForMultiPlaneOverlay(PGFX_ADAPTER_INFO pAdapterInfo, PPLANES pPlanes);
 ...
 */
CDLL_EXPORT BOOL mainline_GetMPOCaps(PGFX_ADAPTER_INFO pAdapterInfo, PMPO_CAPS_ARGS pMPOCaps);
 ...
 * @return			Returns 0(False) on function failure; else 1(True)
 */
CDLL_EXPORT BOOL mainline_GetMPOGroupCaps(PGFX_ADAPTER_INFO pAdapterInfo, PMPO_GROUP_CAPS_ARGS pMPOGroupCaps, UINT uiGroupIndex);
 ...
 * @return		Returns 1(True) if hardware supports the passed plane info, else 1 for check MPO failure and 2 for resource allocation failure
 */
CDLL_EXPORT UINT mainline_CheckForMultiPlaneOverlaySupport3(PGFX_ADAPTER_INFO pAdapterInfo, PPLANES pPlanes);
 ...
 * @return		Returns 1 for SSA MPO failure and 2 for resource allocation failure; else 1(True)
 */
CDLL_EXPORT UINT mainline_SetSourceAddressForMultiPlaneOverlay3(PGFX_ADAPTER_INFO pAdapterInfo, PPLANES pPlanes);
 ...
 * @return		Returns 0(False) on function failure; else 1(True)
 */
CDLL_EXPORT BOOL mainline_EnableDisableMPODFT(PGFX_ADAPTER_INFO pAdapterInfo, HANDLE hGfxValSim, BOOL bEnable, SIMDRIVER_DISPLAY_FEATURE eFeature);
