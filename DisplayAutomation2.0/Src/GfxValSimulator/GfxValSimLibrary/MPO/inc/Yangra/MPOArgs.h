...
typedef struct _DD_FLIP_MPO_PLANE_IN_FLAGS
{
    union {
        struct
        {
            ...
            ULONG StereoFlip : 1;
            ULONG FlipImmediateNoTearing : 1;
            ULONG Reserved : 25;
        };
        ULONG Value;
    };
...
typedef struct _DD_FLIP_MPO_PLANE
{
    ...
    ULONG                        NumGeneratedFrames;
    ULONG                        PresentSeqNumber;
} DD_FLIP_MPO_PLANE;
...
typedef struct _DD_ARG_FLIP_MPO
{
    ...
    IN ULONG64 TargetFlipTime;
    IN BOOLEAN bDriverInternalFlip;
