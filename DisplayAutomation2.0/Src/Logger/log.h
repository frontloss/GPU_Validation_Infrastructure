...
#define DISPCONFIG_LOG(...) DisplayConfigLog(__FUNCTION__, __LINE__, __VA_ARGS__)
...
extern "C"
{
    ...
    CDLL_EXPORT void DisplayConfigLog(const char *pFunctionName, int line, const char *pFmtString, ...);
#ifdef __cplusplus
}
#endif
// CDLL_EXPORT void Log(const char *pDllName, int level, const char *pFunctionName, int line, const char *pFmtString, ...);
...
CDLL_EXPORT void InitDisplayConfigLogger(const char *pFilePath);
